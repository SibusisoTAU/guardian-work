...
export default function BusinessDashboard() {
  ...
  useEffect(() => {
    async function checkProfile() {
      ...
      const { data: biz } = await supabase.from('businesses').select('*').eq('auth_user_id', user.id).single()
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-4xl mx-auto bg-white rounded-2xl p-8 shadow">
        ...
        <p className="text-gray-500 mt-2">{business?.province} | {business?.town}</p>
        <div className="mt-6 p-4 bg-green-50 rounded-xl">✅ Profile complete!</div>
      </div>
    </div>
  )
}
