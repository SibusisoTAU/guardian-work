  ...
  const renderWork = () => {
    if (account?.role === "business") {
      if (!currentBusiness) {
        return (
          <div className="space-y-6">
            <Card className="border-orange-200 bg-white p-6 shadow-sm">
              <div className="text-xs font-black uppercase tracking-widest text-orange-500">I'm Hiring</div>
              <h1 className="mt-2 text-3xl font-black text-slate-900">Business profile needs attention</h1>
              <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">We know this account is a business, but its Business Identity has not loaded yet. GUARDIAN WORK will not show you a Job Seeker profile.</p>
              <div className="mt-5 rounded-2xl bg-orange-50 p-4 text-sm font-semibold text-orange-900">Your business information is being recovered. Refresh the business workspace or complete your Business Identity.</div>
              <div className="mt-5 flex flex-wrap gap-3">
                <Button variant="orange" onClick={() => { void loadAll(); }}>Refresh Business Profile</Button>
                <Button variant="light" onClick={() => nav("business")}>Open Business Workspace</Button>
              </div>
            </Card>
          </div>
        );
