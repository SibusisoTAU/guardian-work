...
export default function Page() {
      ...
      <div className="mx-auto flex min-h-[85vh] max-w-md items-center">
        <Card className="w-full overflow-hidden">
          <div className="bg-emerald-700 p-7 text-white">
            ...
            <div className="mt-5 text-xs font-black uppercase tracking-widest text-orange-300">GUARDIAN WORK V6.6.2</div>
      ...
      <Card className="overflow-hidden border-emerald-200">
        <div className="bg-gradient-to-r from-emerald-700 to-emerald-600 p-6 text-white">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <div className="text-xs font-black uppercase tracking-widest text-emerald-100">GUARDIAN WORK ID · V6.6.2</div>
  ...
  const renderMatches = () => (
    <div className="space-y-6">
      <div>
        <div className="text-xs font-black uppercase tracking-widest text-orange-500">V6.6.2 Match Engine</div>
          ...
          <div className="p-5">
            ...
            <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <label className="cursor-pointer rounded-xl border border-emerald-200 bg-white px-3 py-2 text-xs font-black text-emerald-700">
                  📎 Add media
                  <input type="file" multiple accept="image/*,application/pdf,video/*" className="hidden" disabled={postUploadBusy} onChange={e => { const files = Array.from(e.target.files || []); if (files.length > 4) return flash("You can attach up to 4 files per post."); if (files.some(f => f.size > 25 * 1024 * 1024)) return flash("Each attachment must be 25MB or smaller."); setPostFiles(files); }} />
                </label>
                <span className="text-[10px] font-bold text-slate-400">Pictures · PDF · Video · up to 4 files</span>
              </div>
              {postFiles.length > 0 && <div className="mt-3 flex flex-wrap gap-2">{postFiles.map((file, i) => <div key={`${file.name}-${i}`} className="flex items-center gap-2 rounded-xl bg-emerald-50 px-3 py-2 text-xs font-bold text-emerald-800"><span>{file.type.startsWith("video/") ? "🎥" : file.type === "application/pdf" ? "📄" : "🖼️"}</span><span className="max-w-[180px] truncate">{file.name}</span><button type="button" onClick={() => setPostFiles(old => old.filter((_, idx) => idx !== i))} className="font-black text-rose-600">×</button></div>)}</div>}
              <Button variant="orange" disabled={postUploadBusy || (!postText.trim() && !postFiles.length)} onClick={createPost}>{postUploadBusy ? "Publishing..." : "Publish to Timeline"}</Button>
            </div>
          </div>
  ...
  const renderAlerts = () => {
    ...
    return (
      <div className="space-y-6">
        <div>
          <div className="text-xs font-black uppercase tracking-widest text-orange-500">V6.6.2 - GUARDIAN ALERTS</div>
  ...
  const renderSettings = () => (
    <div className="space-y-6">
      ...
      <div className="text-xs text-slate-400">GUARDIAN WORK V6.6.2 - Media Profiles + Timeline Attachments + Advertising</div>
    </div>
  );
