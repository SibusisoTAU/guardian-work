...
export default function Page() {
  ...
  const [talentProfileLoading, setTalentProfileLoading] = useState(false);
  const [talentContactOpen, setTalentContactOpen] = useState(false);
  const [talentContactMessage, setTalentContactMessage] = useState("Hi, I found your GUARDIAN WORK profile and would like to discuss a work opportunity with you.");
  const [talentContactBusy, setTalentContactBusy] = useState(false);
  ...
  const loadAll = async () => {
    ...
    const [
      ...
      currentBusiness
        ...
        : Promise.resolve({ data: [], error: null }),
      currentBusiness
        ? supabase.from("guardian_contact_requests").select("*").eq("business_id", currentBusiness.id).order("created_at", { ascending: false })
        : currentPerson
          ? supabase.from("guardian_contact_requests").select("*").eq("job_seeker_id", currentPerson.id).order("created_at", { ascending: false })
          : Promise.resolve({ data: [], error: null }),
      currentPerson
  ...
  const viewTalentProfile = async (person: Person) => {
    if (!currentBusiness) return flash("Switch to a business workspace to view talent profiles.");
    setSelectedTalent(person);
    setSelectedTalentWorkId(null);
    setSelectedTalentProofs([]);
    setSelectedTalentRecommendations([]);
    setTalentProfileLoading(true);
    const [workIdRes, proofsRes, recommendationsRes] = await Promise.all([
      supabase.from("guardian_work_ids").select("*").eq("job_seeker_id", person.id).eq("discoverable", true).maybeSingle(),
      supabase.from("guardian_work_proofs").select("*").eq("job_seeker_id", person.id).in("status", ["self_added", "review_pending", "verified"]).order("created_at", { ascending: false }),
    ...
    if (workIdRes.error) flash(workIdRes.error.message);
    setSelectedTalentWorkId((workIdRes.data || null) as WorkID | null);
    setSelectedTalentProofs((proofsRes.data || []) as WorkProof[]);
    setSelectedTalentRecommendations((recommendationsRes.data || []) as WorkRecommendation[]);
    setTalentProfileLoading(false);
  };

  const sendTalentContactRequest = async () => {
    if (!currentBusiness || !selectedTalent) return;
    const message = talentContactMessage.trim();
    if (message.length < 5) return flash("Add a short message before sending the contact request.");
    setTalentContactBusy(true);
    const { data, error } = await supabase.from("guardian_contact_requests").insert({
      business_id: currentBusiness.id,
      job_seeker_id: selectedTalent.id,
      opportunity_id: null,
      message,
      status: "pending",
    }).select().single();
    setTalentContactBusy(false);
    if (error) return flash(error.message);
    setContactRequests(old => [data as ContactRequest, ...old]);
    setTalentContactOpen(false);
    flash("Contact request sent. The job seeker can respond inside GUARDIAN WORK.");
  };

  const updateContactRequestStatus = async (request: ContactRequest, status: "accepted" | "declined") => {
    const { data, error } = await supabase.from("guardian_contact_requests").update({ status, updated_at: new Date().toISOString() }).eq("id", request.id).eq("job_seeker_id", currentPerson?.id || "").select().single();
    if (error) return flash(error.message);
    setContactRequests(old => old.map(x => x.id === request.id ? data as ContactRequest : x));
  ...
  const createPost = async () => {
    if (!postText.trim() || !authUser) return;
    ...
    setPostText("");
    setPostTitle("");
    flash("Published to the GUARDIAN Work Timeline.");
  };
      ...
      <div className="mx-auto flex min-h-[85vh] max-w-md items-center">
        <Card className="w-full overflow-hidden">
          <div className="bg-emerald-700 p-7 text-white">
            ...
            <div className="mt-5 text-xs font-black uppercase tracking-widest text-orange-300">GUARDIAN WORK V6.5.1</div>
    ...
    <div className="min-h-screen bg-slate-50 px-4 py-8">
      <div className="mx-auto flex min-h-[85vh] max-w-md items-center">
        <Card className="w-full overflow-hidden">
          <div className="bg-emerald-700 p-7 text-white">
            <div className="grid h-14 w-14 place-items-center rounded-2xl bg-orange-500 text-xl font-black">GW</div>
            <div className="mt-5 text-xs font-black uppercase tracking-widest text-orange-300">GUARDIAN WORK V6.5</div>
          ...
          <div className="p-6">
            ...
            <div className="mt-5 text-center text-xs text-slate-500">V6.5 keeps Work ID, proof of ability and stronger discovery active.</div>
          </div>
        </Card>
      </div>
    </div>
    ...
    <div className="min-h-screen bg-slate-50 px-4 py-8">
      <div className="mx-auto flex min-h-[85vh] max-w-xl items-center">
        <Card className="w-full p-6 sm:p-8">
          <div className="text-xs font-black uppercase tracking-widest text-orange-500">V6.5 setup</div>
        ...
        <div className="grid gap-8 lg:grid-cols-[1.4fr_.6fr] lg:items-center">
          <div>
            <div className="mb-3 inline-flex rounded-full bg-orange-400 px-3 py-1 text-xs font-black text-slate-950">
              V6.5 - ACTIVE WORK ID + SOCIAL TIMELINE + MATCH ENGINE
            </div>
      ...
      <Card className="overflow-hidden border-emerald-200">
        <div className="bg-gradient-to-r from-emerald-700 to-emerald-600 p-6 text-white">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <div className="text-xs font-black uppercase tracking-widest text-emerald-100">GUARDIAN WORK ID · V6.5</div>
  ...
  const renderMatches = () => (
    <div className="space-y-6">
      <div>
        <div className="text-xs font-black uppercase tracking-widest text-orange-500">V6.5 Match Engine</div>
          ...
          <div className="p-5">
            <div className="flex gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="text-xs text-slate-500">{account?.role === "business" ? "Business account" : currentPerson?.job_title || "Work identity"}</div>
              </div>
            </div>
            <input value={postTitle} onChange={e => setPostTitle(e.target.value)} placeholder="Add a headline (optional)" className="mt-4 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-emerald-500" />
            <textarea value={postText} onChange={e => setPostText(e.target.value)} placeholder="What's happening with your work today? Share your work, skill, availability, achievement or an opportunity..." className="mt-3 min-h-28 w-full resize-none rounded-2xl border border-slate-200 p-4 text-sm outline-none focus:border-emerald-500" />
            <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex flex-wrap gap-2">
                <select value={postType} onChange={e => setPostType(e.target.value)} className="rounded-xl border border-slate-200 px-3 py-2.5 text-xs font-bold">
                  <option value="general">💬 General</option><option value="availability">🟢 I'm available for work</option><option value="work_showcase">🛠️ Show my work</option><option value="experience">💼 My experience</option><option value="opportunity">🟠 Opportunity</option><option value="achievement">🏆 Achievement</option><option value="training">📚 Training / learning</option><option value="success">🎉 Success story</option>
                </select>
                <select value={selectedCircleId} onChange={e => setSelectedCircleId(e.target.value)} className="rounded-xl border border-slate-200 px-3 py-2.5 text-xs font-bold">
              ...
              <Button variant="orange" disabled={!postText.trim()} onClick={createPost}>Publish to Timeline</Button>
            </div>
          </div>
          <div className="grid grid-cols-3 border-t border-slate-100 bg-slate-50 text-center text-[10px] font-black text-slate-500">
            <div className="p-3">📸 Show your work</div><div className="border-x border-slate-100 p-3">💼 Build discoverability</div><div className="p-3">🟠 Create opportunity</div>
          </div>
        </Card>
              ...
              <Card className="overflow-hidden">
                <div className="p-5">
                  ...
                  <div className="mt-3 flex flex-col gap-2 sm:flex-row">
                    <input value={commentDrafts[post.id] || ""} onChange={e => setCommentDrafts(old => ({ ...old, [post.id]: e.target.value }))} placeholder="Write a useful work-related comment..." className="min-w-0 flex-1 rounded-xl border border-slate-200 px-3 py-2.5 text-xs outline-none" />
  ...
  const renderBusiness = () => (
    <div className="space-y-6">
      ...
      <Card className="p-6 border-emerald-200">
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {businessFilteredPeople.slice(0,12).map(p=>{
            const saved = businessShortlists.some(s => s.job_seeker_id === p.id);
            return <Card key={p.id} className="p-4"><div className="flex gap-3"><div className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-emerald-100 font-black text-emerald-700">{initials(p.name)}</div><div className="min-w-0"><div className="font-black text-slate-900">{p.name || "Job seeker"}</div><div className="text-sm font-bold text-orange-600">{p.job_title || "Work seeker"}</div><div className="text-xs text-slate-500">{[p.town,p.province].filter(Boolean).join(" - ")}</div></div></div>{p.skills&&<div className="mt-3 text-xs text-slate-600">{p.skills}</div>}<div className="mt-3 flex flex-wrap items-center gap-2"><span className="rounded-full bg-emerald-50 px-2 py-1 text-[10px] font-black text-emerald-700">Discoverable</span><Button variant="green" onClick={()=>viewTalentProfile(p)}>View profile</Button><Button variant="light" onClick={()=>shortlistPerson(p)}>{saved ? "Saved" : "Save talent"}</Button><Button variant="orange" onClick={()=>{setSelectedTalent(p);setTalentContactMessage(`Hi ${p.name || "there"}, I found your GUARDIAN WORK profile and would like to discuss a work opportunity with you.`);setTalentContactOpen(true);}}>Contact</Button></div></Card>;
          })}
  ...
  const renderOperations = () => {
    ...
    return (
      ...
      <Card className="p-6 border-orange-200">
        ...
        </div>{account?.role === "business" && currentBusiness ? <Button className="mt-4" variant="orange" disabled={adCampaignBusy} onClick={startAdCampaign}>{adCampaignBusy ? "Saving..." : `Create R${(AD_PACKAGES.find(x=>x.code===adPackage)||AD_PACKAGES[0]).price}/day campaign`}</Button> : <div className="mt-4 rounded-xl bg-slate-50 p-3 text-xs font-bold text-slate-500">Business account required to create sponsored ads.</div>}
  ...
  const renderAlerts = () => {
    ...
    return (
      <div className="space-y-6">
        <div>
          <div className="text-xs font-black uppercase tracking-widest text-orange-500">V6.6.1 - GUARDIAN ALERTS</div>
        ...
        {currentPerson && <Card className="p-6 border-orange-200">
          <div className="flex flex-wrap items-center justify-between gap-3"><div><div className="text-xs font-black uppercase tracking-widest text-orange-500">Talent Contact Requests</div><h2 className="mt-1 text-xl font-black text-slate-900">Businesses that want to connect.</h2><p className="mt-1 text-xs text-slate-500">Review the business and message before you accept.</p></div><span className="rounded-full bg-orange-50 px-3 py-1 text-xs font-black text-orange-700">{contactRequests.filter(r=>r.status === "pending").length} pending</span></div>
          <div className="mt-4 space-y-3">{contactRequests.map(r=>{const business=businesses.find(b=>b.id===r.business_id); return <div key={r.id} className="rounded-2xl border border-slate-200 p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><div className="font-black text-slate-900">{business?.name || business?.business_name || "Business"}</div><div className="mt-1 text-xs text-slate-500">{[business?.town,business?.province].filter(Boolean).join(" - ") || "Business workspace"}</div><p className="mt-3 text-sm leading-6 text-slate-600">{r.message}</p></div><span className="w-fit rounded-full bg-slate-100 px-3 py-1 text-[10px] font-black uppercase text-slate-600">{r.status}</span></div>{r.status === "pending" && <div className="mt-4 flex flex-wrap gap-2"><Button variant="green" onClick={()=>updateContactRequestStatus(r,"accepted")}>Accept</Button><Button variant="light" onClick={()=>updateContactRequestStatus(r,"declined")}>Decline</Button></div>}</div>;})}{!contactRequests.length&&<div className="rounded-2xl bg-slate-50 p-5 text-sm text-slate-500">No employer contact requests yet.</div>}</div>
        </Card>}
  ...
  const renderSettings = () => (
    <div className="space-y-6">
      ...
      <Card className="p-6 border-emerald-200"><div className="text-xs font-black uppercase tracking-widest text-orange-500">About GUARDIAN WORK</div><h2 className="mt-1 text-2xl font-black text-slate-900">Make Your Ability Discoverable.</h2><p className="mt-2 text-sm leading-6 text-slate-500">GUARDIAN WORK is a product of MY GUARDIAN LINK, built to connect people's abilities with real work opportunities.</p><div className="mt-4 flex flex-wrap items-center gap-3"><span className="rounded-full bg-emerald-50 px-3 py-2 text-xs font-black text-emerald-700">MY GUARDIAN LINK</span><a href="mailto:mtausibusiso@gmail.com?subject=GUARDIAN%20WORK" className="rounded-xl bg-orange-500 px-4 py-2.5 text-xs font-black text-white">Email Us</a><span className="text-xs text-slate-500">mtausibusiso@gmail.com</span></div></Card>
      ...
      <div className="text-xs text-slate-400">GUARDIAN WORK V6.6.1 - Active Talent Profiles - Contact Requests - Security + Data</div>
    </div>
  );

  const renderTalentProfileModal = () => {
    if (!selectedTalent) return null;
    return <div className="fixed inset-0 z-[70] overflow-y-auto bg-slate-950/60 p-4" onClick={()=>setSelectedTalent(null)}>
      <div className="mx-auto my-6 max-w-3xl" onClick={e=>e.stopPropagation()}>
        <Card className="overflow-hidden">
          <div className="bg-emerald-700 p-6 text-white sm:p-8"><div className="flex flex-wrap items-center gap-4"><div className="grid h-16 w-16 place-items-center rounded-full bg-orange-100 text-xl font-black text-orange-700">{initials(selectedTalent.name)}</div><div><div className="text-2xl font-black">{selectedTalent.name || "GUARDIAN WORK member"}</div><div className="font-bold text-orange-300">{selectedTalent.job_title || "Work identity"}</div><div className="mt-1 text-xs text-emerald-100">{[selectedTalent.town,selectedTalent.province].filter(Boolean).join(" - ")}</div></div></div></div>
          <div className="space-y-5 p-6">
            {talentProfileLoading ? <div className="py-10 text-center text-sm font-bold text-slate-500">Loading Work Profile...</div> : <>
              <div><div className="text-xs font-black uppercase tracking-widest text-emerald-600">Work Identity</div><h3 className="mt-1 text-xl font-black">{selectedTalentWorkId?.headline || selectedTalent.headline || selectedTalent.job_title || "Work seeker"}</h3><p className="mt-2 text-sm leading-6 text-slate-600">{selectedTalentWorkId?.summary || selectedTalent.experience || "This person has not added a Work ID summary yet."}</p></div>
              <div><div className="text-xs font-black uppercase tracking-widest text-emerald-600">Skills</div><div className="mt-2 flex flex-wrap gap-2">{safeText(selectedTalent.skills).split(",").map(x=>x.trim()).filter(Boolean).map(x=><span key={x} className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-bold text-emerald-700">{x}</span>)}{!safeText(selectedTalent.skills).trim()&&<span className="text-sm text-slate-500">No skills added yet.</span>}</div></div>
              <div className="grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-400">Experience</div><div className="mt-1 text-sm font-bold text-slate-800">{selectedTalent.experience || "Not specified"}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-400">Availability</div><div className="mt-1 text-sm font-bold text-slate-800">{selectedTalent.availability || "Not specified"}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-400">Profile completion</div><div className="mt-1 text-sm font-bold text-slate-800">{selectedTalent.profile_completion || 0}%</div></div></div>
              <div><div className="text-xs font-black uppercase tracking-widest text-emerald-600">Proof of Ability</div><div className="mt-3 space-y-2">{selectedTalentProofs.length ? selectedTalentProofs.slice(0,8).map(proof=><div key={proof.id} className="rounded-xl border border-slate-200 p-3"><div className="font-black text-slate-900">{proof.title}</div><div className="mt-1 text-[10px] font-black uppercase text-orange-600">{proof.proof_type.replace("_"," ")} · {proof.status.replace("_"," ")}</div>{proof.description&&<div className="mt-1 text-xs text-slate-500">{proof.description}</div>}</div>) : <div className="rounded-xl bg-slate-50 p-4 text-sm text-slate-500">No public proof has been added yet.</div>}</div></div>
              <div><div className="text-xs font-black uppercase tracking-widest text-emerald-600">Recommendations</div><div className="mt-3 space-y-2">{selectedTalentRecommendations.length ? selectedTalentRecommendations.slice(0,5).map(r=><div key={r.id} className="rounded-xl border border-slate-200 p-3"><div className="text-sm font-bold text-slate-700">{r.message}</div><div className="mt-1 text-[10px] font-black uppercase text-slate-400">{r.relationship || "Recommendation"}</div></div>) : <div className="rounded-xl bg-slate-50 p-4 text-sm text-slate-500">No recommendations yet.</div>}</div></div>
              <div className="rounded-2xl border border-orange-200 bg-orange-50 p-4"><div className="text-sm font-black text-slate-900">Ready to connect?</div><div className="mt-1 text-xs leading-5 text-slate-600">Contacting someone does not require a WhatsApp subscription. GUARDIAN WORK sends a real contact request that the person can accept or decline.</div></div>
            </>}
            <div className="flex flex-wrap justify-end gap-2"><Button variant="light" onClick={()=>setSelectedTalent(null)}>Close</Button><Button variant="orange" onClick={()=>{setTalentContactMessage(`Hi ${selectedTalent.name || "there"}, I found your GUARDIAN WORK profile and would like to discuss a work opportunity with you.`);setTalentContactOpen(true);}}>Contact this person</Button></div>
          </div>
        </Card>
      </div>
    </div>;
  ...
  const renderTalentContactModal = () => {
    if (!talentContactOpen || !selectedTalent) return null;
    return <div className="fixed inset-0 z-[80] grid place-items-center bg-slate-950/60 p-4"><Card className="w-full max-w-lg p-6"><div className="text-xs font-black uppercase tracking-widest text-orange-500">GUARDIAN WORK Contact Request</div><h2 className="mt-1 text-2xl font-black text-slate-900">Contact {selectedTalent.name || "this person"}</h2><p className="mt-2 text-sm leading-6 text-slate-500">Send a message inside GUARDIAN WORK. The job seeker decides whether to accept the request.</p><textarea value={talentContactMessage} onChange={e=>setTalentContactMessage(e.target.value)} rows={5} className="mt-4 w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm"/><div className="mt-4 flex justify-end gap-2"><Button variant="light" onClick={()=>setTalentContactOpen(false)}>Cancel</Button><Button variant="orange" disabled={talentContactBusy} onClick={sendTalentContactRequest}>{talentContactBusy ? "Sending..." : "Send contact request"}</Button></div></Card></div>;
  };
    ...
    <div className={`guardian-work ${theme === "dark" ? "guardian-dark" : ""} min-h-screen bg-slate-50 text-slate-900`}>
      ...
      <footer className="border-t border-slate-200 bg-white">
        <div className="mx-auto flex max-w-7xl flex-col gap-2 px-4 py-8 text-xs text-slate-500 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div>V6.5 - No Dead Buttons · Active Actions</div>
        </div>
      </footer>
