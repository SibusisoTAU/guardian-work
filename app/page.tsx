...
export default function Page() {
      ...
      <div className="mx-auto flex min-h-[85vh] max-w-md items-center">
        <Card className="w-full overflow-hidden">
          <div className="bg-emerald-700 p-7 text-white">
            ...
            <div className="mt-5 text-xs font-black uppercase tracking-widest text-orange-300">GUARDIAN WORK V6.6.3</div>
      ...
      <Card className="overflow-hidden border-emerald-200">
        <div className="bg-gradient-to-r from-emerald-700 to-emerald-600 p-6 text-white">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <div className="text-xs font-black uppercase tracking-widest text-emerald-100">GUARDIAN WORK ID · V6.6.3</div>
  ...
  const renderMatches = () => (
    <div className="space-y-6">
      <div>
        <div className="text-xs font-black uppercase tracking-widest text-orange-500">V6.6.3 Match Engine</div>
        ...
        <Card id="guardian-timeline-composer" className="overflow-hidden">
          <div className="p-5">
            ...
            <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="mt-4 rounded-2xl border-2 border-dashed border-emerald-200 bg-emerald-50/60 p-4">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <div className="text-sm font-black text-slate-900">📎 Add to your post</div>
                    <div className="mt-1 text-[11px] font-bold text-slate-500">Share pictures, PDF documents or videos · up to 4 files · 25MB each</div>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <label className="cursor-pointer rounded-xl bg-emerald-600 px-3 py-2.5 text-xs font-black text-white shadow-sm hover:bg-emerald-700">
                      📸 Photo
                      <input type="file" accept="image/*" multiple className="hidden" disabled={postUploadBusy} onChange={e => { const incoming = Array.from(e.target.files || []); if (postFiles.length + incoming.length > 4) return flash("You can attach up to 4 files per post."); if (incoming.some(f => f.size > 25 * 1024 * 1024)) return flash("Each attachment must be 25MB or smaller."); setPostFiles(old => [...old, ...incoming]); e.currentTarget.value = ""; }} />
                    </label>
                    <label className="cursor-pointer rounded-xl bg-orange-500 px-3 py-2.5 text-xs font-black text-white shadow-sm hover:bg-orange-600">
                      📄 PDF
                      <input type="file" accept="application/pdf" multiple className="hidden" disabled={postUploadBusy} onChange={e => { const incoming = Array.from(e.target.files || []); if (postFiles.length + incoming.length > 4) return flash("You can attach up to 4 files per post."); if (incoming.some(f => f.size > 25 * 1024 * 1024)) return flash("Each attachment must be 25MB or smaller."); setPostFiles(old => [...old, ...incoming]); e.currentTarget.value = ""; }} />
                    </label>
                    <label className="cursor-pointer rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-xs font-black text-slate-700 shadow-sm hover:bg-slate-50">
                      🎥 Video
                      <input type="file" accept="video/*" multiple className="hidden" disabled={postUploadBusy} onChange={e => { const incoming = Array.from(e.target.files || []); if (postFiles.length + incoming.length > 4) return flash("You can attach up to 4 files per post."); if (incoming.some(f => f.size > 25 * 1024 * 1024)) return flash("Each attachment must be 25MB or smaller."); setPostFiles(old => [...old, ...incoming]); e.currentTarget.value = ""; }} />
                    </label>
                  </div>
                </div>
                {postFiles.length > 0 && (
                  <div className="mt-4 rounded-xl bg-white p-3">
                    <div className="mb-2 text-[10px] font-black uppercase tracking-wider text-emerald-700">Ready to publish · {postFiles.length}/4</div>
                    <div className="flex flex-wrap gap-2">{postFiles.map((file, i) => <div key={`${file.name}-${i}`} className="flex items-center gap-2 rounded-xl bg-emerald-50 px-3 py-2 text-xs font-bold text-emerald-800"><span>{file.type.startsWith("video/") ? "🎥" : file.type === "application/pdf" ? "📄" : "🖼️"}</span><span className="max-w-[180px] truncate">{file.name}</span><button type="button" onClick={() => setPostFiles(old => old.filter((_, idx) => idx !== i))} className="font-black text-rose-600">×</button></div>)}</div>
                  </div>
                )}
              </div>
              <Button className="mt-4 w-full sm:w-auto" variant="orange" disabled={postUploadBusy || (!postText.trim() && !postFiles.length)} onClick={createPost}>{postUploadBusy ? "Publishing..." : postFiles.length ? `Publish with ${postFiles.length} attachment${postFiles.length === 1 ? "" : "s"}` : "Publish to Timeline"}</Button>
            </div>
          </div>
  ...
  const renderAlerts = () => {
    ...
    return (
      <div className="space-y-6">
        <div>
          <div className="text-xs font-black uppercase tracking-widest text-orange-500">V6.6.3 - GUARDIAN ALERTS</div>
  ...
  const renderSettings = () => (
    <div className="space-y-6">
      ...
      <div className="text-xs text-slate-400">GUARDIAN WORK V6.6.3 - Media Profiles + Timeline Attachments + Advertising</div>
    </div>
  );
    ...
    <div className={`guardian-work ${theme === "dark" ? "guardian-dark" : ""} min-h-screen bg-slate-50 text-slate-900`}>
      ...
      <footer className="border-t border-slate-200 bg-white">
        <div className="mx-auto flex max-w-7xl flex-col gap-2 px-4 py-8 text-xs text-slate-500 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div>V6.6.1 - No Dead Buttons · Active Actions</div>
        </div>
      </footer>
