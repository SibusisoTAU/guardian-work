...
const renderHeader = () => (
  <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur">
    <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3">
      <button onClick={() => nav("home")} className="flex items-center gap-3 text-left">
        ...
        <div>
          <div className="text-lg font-black tracking-t
    ...
    <div className="space-y-6">
        ...
        <div className="grid gap-8 lg:grid-cols-[1.4fr_.6fr] lg:items-center">
          <div>
            <div className="mb-3 inline-flex rounded-full bg-orange-400 px-3 py-1 text-xs font-black text-slate-950">
              V5.8 - LIVE INTELLIGENCE + SMART DISCOVERY
            </div>
      ...
      <Card className="p-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div><div className="text-xs font-black uppercase tracking-widest text-orange-500">V5.8 intelligence</div><h2 className="mt-1 text-xl font-black text-slate-900">Your Smart Matches</h2><p className="mt-1 text-xs text-slate-500">Ranked from your skills, experience, location, availability and Work Identity.</p></div>
  ...
  const renderOperations = () => {
    ...
    return (
      ...
      <Card className="p-6 border-orange-200">
        ...
        </div><Button className="mt-4" variant="orange" onClick={createSponsoredAd}>Create & activate ad</Button>
  ...
  const renderAlerts = () => {
    ...
    return (
      <div className="space-y-6">
        <div>
          <div className="text-xs font-black uppercase tracking-widest text-orange-500">V5.8 - GUARDIAN ALERTS</div>
  ...
  const renderProfile = () => (
    <div className="space-y-6">
      ...
      <Card className="overflow-hidden">
        <div className="bg-emerald-700 p-6 text-white sm:p-8"><div className="flex flex-wrap items-center gap-5"><div className="grid h-20 w-20 place-items-center rounded-full bg-orange-100 text-2xl font-black text-orange-700">{initials(currentPerson?.name || "GW")}</div><div><div className="text-2xl font-black">{currentPerson?.name || "GUARDIAN WORK member"}</div><div className="text-emerald-100">{currentPerson?.job_title || "Work identity"}</div><div className="mt-2 text-xs text-emerald-100">{[currentPerson?.town,currentPerson?.province].filter(Boolean).join(" - ")}</div></div></div></div>
  ...
  const renderSettings = () => (
    <div className="space-y-6">
      ...
      <div className="text-xs text-slate-400">GUARDIAN WORK V5.8 - Live Intelligence - Smart Discovery - Saved Work - Business Shortlists</div>
    </div>
  );
    ...
    <div className={`guardian-work ${theme === "dark" ? "guardian-dark" : ""} min-h-screen bg-slate-50 text-slate-900`}>
      ...
      <footer className="border-t border-slate-200 bg-white">
        <div className="mx-auto flex max-w-7xl flex-col gap-2 px-4 py-8 text-xs text-slate-500 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div>V5.8 - Live Intelligence, Smart Discovery & Alerts</div>
        </div>
      </footer>
