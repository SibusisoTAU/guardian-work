...
type DirectMessage = { id: string; sender_auth_user_id: string; recipient_auth_user_id: string; content: string | null; created_at: string; };
...
export default function Page() {
  ...
  const loadChatMessages = async (partnerAuthId: string) => {
    ...
    setChatPartnerAuthId(partnerAuthId);
    ...
    if (messages.length) {
      ...
      for (const a of (attachments || []) as DirectMessageAttachment[]) {
        ...
        const row = { ...a, public_url: signed?.signedUrl || "" };
        (grouped[a.message_id] ||= []).push(row);
      }
      setChatMessageAttachments(grouped);
    } else setChatMessageAttachments({});
  };
  ...
  const openPrivateChatFromProfile = async (partnerAuthId: string) => {
    ...
    await openChatDialog(person?.name || "GUARDIAN member", partnerAuthId, person?.profile_photo_url || null);
  };
  ...
  const uploadDirectMessageAttachments = async (messageId: string, files: File[]) => {
    ...
    const { data: rows } = await supabase.from("guardian_direct_message_attachments").select("*").eq("message_id", messageId).order("created_at", { ascending: true });
    const signedRows: DirectMessageAttachment[] = [];
    for (const row of (rows || []) as DirectMessageAttachment[]) {
      const { data: signed } = await supabase.storage.from("guardian-chat-media").createSignedUrl(row.file_path, 60 * 60);
      signedRows.push({ ...row, public_url: signed?.signedUrl || "" });
    }
    setChatMessageAttachments(old => ({ ...old, [messageId]: signedRows }));
  ...
  const renderHeader = () => (
    <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur">
      <div className="mx-auto flex max-w-7xl items-center gap-2 px-2 py-2 sm:px-4 sm:py-3">
        <button
          onClick={() => setProfileMenuOpen(v => !v)}
          aria-label="Open GUARDIAN WORK menu"
          aria-expanded={profileMenuOpen}
          className="grid h-10 w-10 shrink-0 place-items-center rounded-xl border border-slate-200 bg-white text-xl font-black text-slate-700 shadow-sm hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-orange-400 sm:h-11 sm:w-11"
        >
          <span aria-hidden="true">☰</span>
        </button>

        <button onClick={() => { setProfileMenuOpen(false); nav("home"); }} className="flex min-w-0 shrink-0 items-center gap-2 text-left">
          <div className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-emerald-600 text-sm font-black text-white shadow-lg shadow-emerald-200 sm:h-11 sm:w-11 sm:rounded-2xl sm:text-base">GW</div>
          <div className="min-w-0">
            <div className="truncate text-[14px] font-black tracking-tight text-slate-900 sm:text-lg">
              GUARDIAN <span className="text-orange-500">WORK</span>
            </div>
            <div className="hidden text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500 sm:block">Make your ability discoverable</div>
          </div>
        </button>

        <nav aria-label="Primary navigation" className="ml-auto min-w-0 flex-1 overflow-x-auto scrollbar-none">
          <div className="flex min-w-max items-center justify-end gap-1">
            {primaryNavItems.map(([id, label]) => (
              <button key={id} onClick={() => { setProfileMenuOpen(false); nav(id); }} className={`shrink-0 rounded-xl px-2.5 py-2 text-[10px] font-black sm:px-3 sm:text-[11px] ${active === id ? "bg-emerald-50 text-emerald-700" : id === "support" ? "text-orange-600 hover:bg-orange-50" : "text-slate-600 hover:bg-slate-50"}`}>
                {label}
              </button>
            ))}
          </div>
        ...
        <div className="hidden items-center gap-2 sm:flex">
          <div className="hidden rounded-full bg-emerald-50 px-2.5 py-1 text-[10px] font-black text-emerald-700 md:block">LIVE {liveUsers}</div>
          {currentPerson && <button onClick={() => { setProfileMenuOpen(false); nav("profile"); }} className="text-right" aria-label="Open profile"><div className="text-xs font-black text-slate-900">{currentPerson.name}</div><div className="text-[10px] text-slate-500">{currentPerson.job_title || "Work identity"}</div></button>}
        ...
        {profileMenuOpen && (
          <>
            <button aria-label="Close menu" onClick={() => setProfileMenuOpen(false)} className="fixed inset-0 top-[58px] z-40 cursor-default bg-slate-950/10 sm:top-[68px]" />
            <div className="absolute left-2 top-[58px] z-50 w-[min(92vw,360px)] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl sm:left-4 sm:top-[68px]">
              <div className="border-b border-slate-100 bg-gradient-to-r from-emerald-50 to-orange-50 p-4">
                <div className="text-[10px] font-black uppercase tracking-[0.18em] text-emerald-700">GUARDIAN WORK MENU</div>
                <div className="mt-1 text-sm font-black text-slate-900">Everything you need, one tap away.</div>
              </div>
              <div className="max-h-[calc(100dvh-150px)] overflow-y-auto p-2">
                {navItems.filter(([id]) => !primaryNavItems.some(([primary]) => primary === id)).map(([id, label]) => (
                  <button key={id} onClick={() => { setProfileMenuOpen(false); nav(id); }} className={`flex w-full items-center justify-between rounded-xl px-3 py-3 text-left text-sm font-black ${active === id ? "bg-emerald-50 text-emerald-700" : "text-slate-700 hover:bg-slate-50"}`}>
                    <span>{label}</span><span className="text-slate-300">›</span>
                  </button>
                ))}
                <div className="my-2 border-t border-slate-100" />
                <button onClick={() => { setProfileMenuOpen(false); signOut(); }} className="flex w-full rounded-xl px-3 py-3 text-left text-sm font-black text-red-600 hover:bg-red-50">Sign out</button>
              </div>
            </div>
          </>
        )}
  ...
  const renderChats = () => {
    const peopleContacts = people.filter(p => p.auth_user_id && p.auth_user_id !== authUser?.id).map(p => ({ id: p.id, name: p.name || "GUARDIAN member", authId: p.auth_user_id!, photo: p.profile_photo_url || null, subtitle: p.job_title || "Work identity", type: "person" }));
    const businessContacts = businesses.filter(b => b.auth_user_id && b.auth_user_id !== authUser?.id).map(b => ({ id: b.id, name: b.name || b.business_name || "GUARDIAN business", authId: b.auth_user_id!, photo: null, subtitle: "Business", type: "business" }));
    const filtered = [...peopleContacts, ...businessContacts].filter(c => c.name.toLowerCase().includes(chatSearch.toLowerCase()) || c.subtitle.toLowerCase().includes(chatSearch.toLowerCase())).slice(0, 50);
    const attachmentView = (a: DirectMessageAttachment) => a.file_type.startsWith("image/") ? <a href={a.public_url} target="_blank" rel="noreferrer"><img src={a.public_url} alt={a.file_name} className="mt-2 max-h-64 max-w-full rounded-xl object-cover" /></a> : a.file_type.startsWith("video/") ? <video controls preload="metadata" src={a.public_url} className="mt-2 max-h-64 max-w-full rounded-xl bg-black" /> : <a href={a.public_url} target="_blank" rel="noreferrer" className="mt-2 flex items-center gap-2 rounded-xl border border-orange-200 bg-orange-50 p-3"><span>📎</span><span className="font-black">{a.file_name}</span></a>;
    return (
      <div className="mx-auto max-w-5xl space-y-5">
        <div className="rounded-3xl bg-emerald-700 p-6 text-white shadow-xl"><div className="text-xs font-black uppercase tracking-[0.2em] text-orange-300">GUARDIAN WORK · PRIVATE CHAT</div><h1 className="mt-2 text-3xl font-black">Talk directly. Work privately.</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-emerald-50">Tap any person or business. Their private WhatsApp-style dialogue opens immediately. Send text, pictures, videos and files in the same conversation.</p></div>
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map(c => <button key={`${c.type}-${c.id}`} type="button" onClick={()=>void openChatDialog(c.name,c.authId,c.photo)} className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white p-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-orange-300 hover:shadow-md">
            {c.photo ? <img src={c.photo} alt={c.name} className="h-14 w-14 shrink-0 rounded-full object-cover" /> : <div className="grid h-14 w-14 shrink-0 place-items-center rounded-full bg-emerald-100 text-sm font-black text-emerald-700">{initials(c.name)}</div>}
            <div className="min-w-0"><div className="truncate font-black text-slate-900">{c.name}</div><div className="text-xs font-bold text-orange-600">{c.subtitle}</div><div className="mt-1 text-[10px] font-bold text-slate-400">Tap to open private chat →</div></div>
          </button>)}
        </div>
        ...
        {chatOpen && <div className="fixed inset-0 z-[80] bg-slate-950/60 p-0 sm:p-6" role="dialog" aria-modal="true">
          <div className="mx-auto flex h-full max-w-2xl flex-col overflow-hidden bg-white shadow-2xl sm:h-[92vh] sm:rounded-3xl">
            <div className="flex items-center gap-3 bg-emerald-700 p-4 text-white"><button type="button" onClick={()=>setChatOpen(false)} className="grid h-10 w-10 place-items-center rounded-full bg-white/10 text-xl">←</button>{chatPartnerPhoto?<img src={chatPartnerPhoto} alt={chatPartnerName} className="h-11 w-11 rounded-full object-cover ring-2 ring-orange-300"/>:<div className="grid h-11 w-11 place-items-center rounded-full bg-orange-100 text-xs font-black text-orange-700">{initials(chatPartnerName)}</div>}<div className="min-w-0 flex-1"><div className="truncate font-black">{chatPartnerName}</div><div className="text-[10px] font-bold text-emerald-100">PRIVATE · only you and this person/business</div></div><button type="button" onClick={()=>void loadChatMessages(chatPartnerAuthId)} className="rounded-xl bg-white/10 px-3 py-2 text-[10px] font-black">Refresh</button></div>
            <div className="flex-1 space-y-2 overflow-y-auto bg-[#f5f7f5] p-4">
              {chatMessages.map(m=><div key={m.id} className={`flex ${m.sender_auth_user_id===authUser?.id?"justify-end":"justify-start"}`}><div className={`max-w-[88%] rounded-2xl px-4 py-3 text-sm shadow-sm ${m.sender_auth_user_id===authUser?.id?"bg-emerald-600 text-white":"bg-white text-slate-700"}`}>{m.content&&<div className="whitespace-pre-wrap leading-6">{m.content}</div>}{(chatMessageAttachments[m.id]||[]).map(attachmentView)}<div className={`mt-1 text-[9px] ${m.sender_auth_user_id===authUser?.id?"text-emerald-100":"text-slate-400"}`}>{new Date(m.created_at).toLocaleTimeString("en-ZA",{hour:"2-digit",minute:"2-digit"})}</div></div></div>)}
              {!chatMessages.length&&<div className="grid h-full place-items-center text-center text-slate-400"><div><div className="text-5xl">💬</div><div className="mt-2 font-black">Start the conversation with {chatPartnerName}.</div></div></div>}
            </div>
            <div className="border-t border-slate-200 bg-white p-3">
              {chatFiles.length>0&&<div className="mb-2 flex flex-wrap gap-2">{chatFiles.map((f,i)=><div key={`${f.name}-${i}`} className="rounded-xl bg-slate-100 px-3 py-2 text-[10px] font-bold">{f.name}<button type="button" onClick={()=>setChatFiles(old=>old.filter((_,idx)=>idx!==i))} className="ml-2 text-red-500">×</button></div>)}</div>}
              <div className="flex items-end gap-2"><label className="cursor-pointer rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-3 text-sm font-black text-emerald-700">📎<input type="file" multiple accept="image/*,video/*,application/pdf,.doc,.docx,.txt" className="hidden" disabled={chatBusy} onChange={e=>{const incoming=Array.from(e.target.files||[]); if(chatFiles.length+incoming.length>4)return flash("You can attach up to 4 files per message."); if(incoming.some(f=>f.size>25*1024*1024))return flash("Each chat file must be 25MB or smaller."); setChatFiles(old=>[...old,...incoming]); e.currentTarget.value="";}} /></label><textarea value={chatDraft} onChange={e=>setChatDraft(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();void sendDirectMessage();}}} placeholder="Write a private message..." className="min-h-12 max-h-32 min-w-0 flex-1 resize-none rounded-xl border border-slate-200 px-3 py-3 text-sm outline-none"/><Button variant="orange" disabled={chatBusy||(!chatDraft.trim()&&!chatFiles.length)} onClick={()=>void sendDirectMessage()}>{chatBusy?"Sending...":"Send"}</Button></div>
              <div className="mt-2 text-[10px] font-bold text-slate-400">📷 Pictures · 🎥 videos · 📄 PDF/DOC/DOCX/TXT · up to 4 files · 25MB each</div>
            </div>
          </div>
        </div>}
      </div>
    ...
    return (
      <div className="mx-auto max-w-3xl space-y-5 pb-8">
        ...
        {statuses.map(status=>{
          ...
          return <Card key={status.id} className="overflow-hidden"><div className="p-5">
  ...
  const renderSettings = () => (
    <div className="space-y-6">
      ...
      <div className="text-xs text-slate-400">GUARDIAN WORK V6.9.2 - Command Menu + Status + Chat</div>
    </div>
  );
  ...
  return (
    <div className={`guardian-work ${theme === "dark" ? "guardian-dark" : ""} min-h-screen bg-slate-50 text-slate-900`}>
      ...
      {supportModalOpen && (
        <div className="fixed inset-0 z-[80] overflow-y-auto bg-slate-950/60 p-3 sm:grid sm:place-items-center sm:p-4">
          <div className="my-3 w-full max-w-md max-h-[calc(100dvh-1.5rem)] overflow-y-auto rounded-2xl bg-white p-4 shadow-2xl sm:my-0 sm:max-h-[90dvh] sm:rounded-3xl sm:p-5">
            <div className="flex items-start justify-between gap-4"><div><div className="text-xs font-black uppercase tracking-widest text-emerald-600">GUARDIAN SUPPORT</div><h2 className="mt-1 text-xl font-black leading-tight text-slate-900 sm:text-2xl">Set up your support request</h2><p className="mt-2 text-sm text-slate-500">Your request is recorded first. For card payments, GUARDIAN opens secure Paystack checkout. Payment is only marked paid after provider verification.</p></div><button onClick={()=>setSupportModalOpen(false)} className="text-xl font-black text-slate-400">X</button></div>
      ...
      {adPaymentOpen && (
        <div className="fixed inset-0 z-[75] overflow-y-auto bg-slate-950/60 p-3 sm:grid sm:place-items-center sm:p-4">
          <div className="my-3 w-full max-w-md max-h-[calc(100dvh-1.5rem)] overflow-y-auto rounded-2xl bg-white p-4 shadow-2xl sm:my-0 sm:max-h-[90dvh] sm:rounded-3xl sm:p-5">
            <div className="flex items-start justify-between gap-4"><div><div className="text-xs font-black uppercase tracking-widest text-orange-500">Sponsored Timeline</div><h2 className="mt-1 text-xl font-black leading-tight text-slate-900 sm:text-2xl">Choose how to pay for your campaign</h2><p className="mt-2 text-sm text-slate-500">Your campaign is saved as pending. No payment is claimed here until a real payment provider confirms it.</p></div><button onClick={()=>setAdPaymentOpen(false)} className="text-xl font-black text-slate-400">X</button></div>
            ...
            <div className="mt-5 grid gap-2">{[["credit_card","Credit or debit card",true],["eft","EFT / bank transfer",false],["voucher","Voucher",false],["airtime","Airtime / SIM billing",false]].map(([value,label,live])=><button key={String(value)} disabled={!live} onClick={()=>live && setAdPaymentMethod(String(value))} className={`rounded-xl border p-3 text-left ${!live?"cursor-not-allowed opacity-50 bg-slate-50":adPaymentMethod===value?"border-orange-500 bg-orange-50":"border-slate-200"}`}><div className="font-black text-slate-900">{label}</div><div className="mt-1 text-xs text-slate-500">{live ? "Secure Paystack checkout" : "Not connected yet"}</div></button>)}</div>
      ...
      {paymentOpen && (
        <div className="fixed inset-0 z-[70] overflow-y-auto bg-slate-950/60 p-3 sm:grid sm:place-items-center sm:p-4">
          <div className="my-3 w-full max-w-md max-h-[calc(100dvh-1.5rem)] overflow-y-auto rounded-2xl bg-white p-4 shadow-2xl sm:my-0 sm:max-h-[90dvh] sm:rounded-3xl sm:p-5">
            <div className="flex items-start justify-between gap-4"><div><div className="text-xs font-black uppercase tracking-widest text-orange-500">WhatsApp Alerts</div><h2 className="mt-1 text-xl font-black leading-tight text-slate-900 sm:text-2xl">Choose how to pay R10/month</h2><p className="mt-2 text-sm text-slate-500">Choose Credit or debit card to open secure checkout. WhatsApp access is activated only after the payment provider confirms the transaction.</p></div><button onClick={()=>setPaymentOpen(false)} className="text-xl font-black text-slate-400">X</button></div>
            <div className="mt-5 grid gap-2">
              {[['credit_card','Credit or debit card','Secure Paystack checkout'],['eft','EFT / bank transfer','Secure EFT through Paystack/Ozow if enabled'],['voucher','Voucher','Redeem a GUARDIAN WORK voucher code'],['airtime','Airtime / SIM billing','Create a SIM billing request; provider confirmation required']].map(([value,label,desc])=><button key={String(value)} onClick={()=>setPaymentMethod(String(value))} className={`rounded-xl border p-3 text-left ${paymentMethod===value?'border-orange-500 bg-orange-50':'border-slate-200 hover:bg-slate-50'}`}><div className="font-black text-slate-900">{label}</div><div className="mt-1 text-xs text-slate-500">{desc}</div></button>)}
            </div>
            {paymentMethod === "voucher" && <input value={voucherCode} onChange={e=>setVoucherCode(e.target.value.toUpperCase())} placeholder="Enter voucher code" className="mt-3 w-full rounded-xl border border-slate-200 px-3 py-3 text-sm font-black tracking-wider" />}
