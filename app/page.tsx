...
export default function Page() {
  ...
  const loadAll = async () => {
    ...
    const warnings = [
      ...
      ["Support", supportRequestsRes.error],
  ...
  const startPaystackCheckout = async (args: { requestId: string; amountCents: number; supportType?: string; whatsapp?: boolean }) => {
    if (!authUser?.email) return flash("Your account needs an email address before card payment can start.");
    setPaymentProcessing(true);
    try {
      const session = await supabase.auth.getSession();
      const accessToken = session.data.session?.access_token;
      if (!accessToken) return flash("Your secure session has expired. Please sign in again.");
      const res = await fetch("/api/paystack/initialize", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ accessToken, requestId: args.requestId, email: authUser.email, amountCents: args.amountCents, supportType: args.supportType || null, whatsapp: !!args.whatsapp }),
      });
      const body = await res.json().catch(() => ({}));
      if (!res.ok || !body.authorization_url) return flash(body.error || "Card checkout could not be started. Check the Paystack server configuration.");
      window.location.assign(body.authorization_url);
    } catch (e: any) {
      flash(e?.message || "Card checkout could not be started.");
    } finally {
  ...
  const confirmWhatsAppPaymentMethod = async () => {
    ...
    if (paymentMethod !== "credit_card") return flash("Only credit/debit card checkout is connected right now. Choose Credit or debit card to continue.");
    setPaymentProcessing(true);
    const { data, error } = await supabase.from("guardian_whatsapp_payment_requests").insert({ job_seeker_id: currentPerson.id, amount_cents: 1000, payment_method: paymentMethod, status: "pending" }).select().single();
    if (error) { setPaymentProcessing(false); return flash(error.message); }
    await saveNotificationPreferences({ whatsapp_enabled: true, whatsapp_opt_in: true, whatsapp_subscription_status: "pending", whatsapp_payment_method: "credit_card", whatsapp_payment_status: "pending" } as any);
    await startPaystackCheckout({ requestId: data.id, amountCents: 1000, whatsapp: true });
    return;
  };
      ...
      <div className="mx-auto flex min-h-[85vh] max-w-md items-center">
        <Card className="w-full overflow-hidden">
          <div className="bg-emerald-700 p-7 text-white">
            ...
            <div className="mt-5 text-xs font-black uppercase tracking-widest text-orange-300">GUARDIAN WORK V6.8</div>
      ...
      <div className="mx-auto flex min-h-[85vh] max-w-md items-center">
        <Card className="w-full overflow-hidden">
          ...
          <div className="p-6">
            ...
            <div className="mt-5 text-center text-xs text-slate-500">V6.8 keeps Work ID, proof of ability, matching and launch-ready actions active.</div>
          </div>
        </Card>
      </div>
    </div>
    ...
    <div className="min-h-screen bg-slate-50 px-4 py-8">
      <div className="mx-auto flex min-h-[85vh] max-w-xl items-center">
        <Card className="w-full p-6 sm:p-8">
          <div className="text-xs font-black uppercase tracking-widest text-orange-500">V6.8 setup</div>
        ...
        <div className="grid gap-8 lg:grid-cols-[1.4fr_.6fr] lg:items-center">
          <div>
            <div className="mb-3 inline-flex rounded-full bg-orange-400 px-3 py-1 text-xs font-black text-slate-950">
              V6.8 - SUPPORT + MEDIA TIMELINE + MATCH ENGINE
            </div>
      ...
      <Card className="overflow-hidden border-emerald-200">
        <div className="bg-gradient-to-r from-emerald-700 to-emerald-600 p-6 text-white">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <div className="text-xs font-black uppercase tracking-widest text-emerald-100">GUARDIAN WORK ID · V6.8</div>
  ...
  const renderMatches = () => (
    <div className="space-y-6">
      <div>
        <div className="text-xs font-black uppercase tracking-widest text-orange-500">V6.8 Match Engine</div>
      ...
      <div className="mx-auto max-w-3xl space-y-5 pb-8">
        ...
        {workCircles.length > 0 && (
          <Card className="overflow-hidden">
            ...
            {selectedCircleId && (() => {
              const selectedCircle = workCircles.find(c => c.id === selectedCircleId);
              const selectedJoined = !!selectedCircle && joinedCircleIds.has(selectedCircle.id);
              return selectedCircle ? (
                <div className="mx-4 mb-2 rounded-2xl border border-emerald-100 bg-emerald-50 p-4">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div><div className="text-xs font-black uppercase tracking-widest text-emerald-700">◉ {selectedCircle.name}</div><div className="mt-1 text-sm font-black text-slate-900">{selectedJoined ? "You are inside this Work Circle." : "Join this circle to participate."}</div><p className="mt-1 text-xs leading-5 text-slate-600">{selectedCircle.description || "Follow conversations, share your work, comment and connect with people in this circle."}</p></div>
                    <span className="rounded-full bg-white px-3 py-2 text-xs font-black text-emerald-700">{selectedCircle.member_count || 0} members</span>
                  </div>
                  {selectedJoined && <div className="mt-3 text-xs font-bold text-emerald-800">The Timeline below is filtered to this circle. Read posts, react, comment and publish into the circle.</div>}
                </div>
              ) : null;
            })()}
              ...
              {workCircles.map(circle => {
                ...
                return (
                  <div key={circle.id} className={`min-w-[220px] rounded-2xl border p-4 ${selected ? "border-emerald-500 bg-emerald-50" : "border-slate-200 bg-white"}`}>
                    ...
                    <button disabled={circleBusy === circle.id} onClick={() => joined ? (() => { setSelectedCircleId(circle.id); setFeedFilter("all"); window.setTimeout(() => document.getElementById("guardian-circle-feed")?.scrollIntoView({ behavior: "smooth", block: "start" }), 50); })() : toggleCircleMembership(circle)} className={`mt-3 w-full rounded-xl px-3 py-2 text-xs font-black ${joined ? "bg-slate-100 text-slate-700" : "bg-emerald-600 text-white"}`}>{circleBusy === circle.id ? "Working..." : joined ? "Joined ✓ · Open feed" : "Join circle"}</button>
                  </div>
                );
              })}
            </div>
        ...
        <div id="guardian-circle-feed" className="scroll-mt-24" />
  ...
  const createSupportRequest = async () => {
    ...
    if (paymentMethod !== "credit_card") return flash("Only credit/debit card checkout is connected right now. Other payment methods will be enabled after their provider connections are added.");
    if (authUser.email == null || !authUser.email.trim()) return flash("Your account needs an email address before card payment can start.");
    setSupportBusy(true);
    const { data, error } = await supabase.from("guardian_support_requests").insert({ user_id: authUser.id, role: account?.role || "job_seeker", support_type: supportType, amount_cents: Math.round(amount * 100), payment_method: paymentMethod, purpose: supportPurpose.trim() || null, status: "pending" }).select().single();
  ...
  const renderSupport = () => (
    <div className="space-y-6">
      <div>
        <div className="text-xs font-black uppercase tracking-widest text-orange-500">GUARDIAN SUPPORT · V6.8</div>
  ...
  const renderAlerts = () => {
    ...
    return (
      <div className="space-y-6">
        <div>
          <div className="text-xs font-black uppercase tracking-widest text-orange-500">V6.8 - GUARDIAN ALERTS</div>
  ...
  const renderSettings = () => (
    <div className="space-y-6">
      ...
      <div className="text-xs text-slate-400">GUARDIAN WORK V6.8 - Support + Media Profiles + Timeline Attachments + Advertising</div>
    </div>
  );
    ...
    <div className={`guardian-work ${theme === "dark" ? "guardian-dark" : ""} min-h-screen bg-slate-50 text-slate-900`}>
      ...
      {supportModalOpen && (
        <div className="fixed inset-0 z-[80] grid place-items-center bg-slate-950/60 p-4">
          <div className="w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl">
            ...
            <div className="mt-4 grid gap-2">{[["credit_card","Credit or debit card",true],["eft","EFT / bank transfer",false],["voucher","Voucher",false],["airtime","Airtime / SIM billing",false]].map(([v,l,live])=><button key={String(v)} disabled={!live} onClick={()=>live && setPaymentMethod(String(v))} className={`rounded-2xl border p-3 text-left ${!live?"cursor-not-allowed opacity-50 bg-slate-50":paymentMethod===v?"border-orange-500 bg-orange-50":"border-slate-200"}`}><div className="text-xs font-black text-slate-900">{l}</div><div className="mt-1 text-[11px] text-slate-500">{live?"Secure checkout":"Not connected yet"}</div></button>)}</div>
            <div className="mt-5 flex gap-2"><Button variant="light" className="flex-1" onClick={()=>setSupportModalOpen(false)}>Cancel</Button><Button variant="orange" className="flex-1" disabled={supportBusy || paymentProcessing} onClick={createSupportRequest}>{supportBusy?"Preparing...":paymentMethod === "credit_card" ? "Continue to secure card payment" : "Create support request"}</Button></div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[70] grid place-items-center bg-slate-950/60 p-4">
          <div className="w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl">
            ...
            <div className="mt-5 grid gap-2">
              {[['credit_card','Credit or debit card',true],['eft','EFT / bank transfer',false],['voucher','Voucher',false],['airtime','Airtime / SIM billing',false]].map(([value,label,live])=><button key={String(value)} disabled={!live} onClick={()=>live && setPaymentMethod(String(value))} className={`rounded-2xl border p-4 text-left ${!live ? 'cursor-not-allowed opacity-50 bg-slate-50' : paymentMethod===value?'border-orange-500 bg-orange-50':'border-slate-200'}`}><div className="font-black text-slate-900">{label}</div><div className="mt-1 text-xs text-slate-500">{live ? 'Secure checkout — payment opens with the provider.' : 'Payment connection coming soon — not available yet.'}</div></button>)}
            </div>
            <div className="mt-5 flex gap-2"><Button variant="light" className="flex-1" onClick={()=>setPaymentOpen(false)}>Cancel</Button><Button variant="orange" className="flex-1" disabled={paymentProcessing} onClick={confirmWhatsAppPaymentMethod}>{paymentProcessing ? "Opening secure checkout..." : paymentMethod === "credit_card" ? "Pay R10 securely" : "Continue"}</Button></div>
          </div>
        </div>
      )}
      ...
      <footer className="border-t border-slate-200 bg-white">
        <div className="mx-auto flex max-w-7xl flex-col gap-2 px-4 py-8 text-xs text-slate-500 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div>V6.8 - Launch Hardening · Active Actions</div>
        </div>
      </footer>
