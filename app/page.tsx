...
export default function Page(){
  ...
  const addPerson=async()=>{if(!form.full_name)return; await supabase.from('people').insert([{...form,status:'Available'}]); setForm({full_name:"",phone:"",capability:"Driver"}); fetchPeople()}
  ...
  const whatsapp=(p:any)=>{const m=`Hi ${p.full_name} 👋 Guardian Work:%0A*${selectedJob.title}* at *${selectedJob.company}*%0A${selectedJob.location} • ${selectedJob.pay}%0A%0AAvailable? Reply YES`; const ph=p.phone.replace(/\D/g,''); window.open(`https://wa.me/${ph}?text=${m}`,'_blank')}

  return(
    <div className="min-h-screen bg-black p-4 pb-24">
      <h1 className="text-white text-xl font-black">GUARDIAN WORK V5</h1><p className="text-green-400 text-xs mb-4">● ENGINE ONLINE - Phase 3.5 Match</p>
      <div className="bg-[#1a1a1a] rounded-[20px] p-4">
        ...
        <select value={form.capability} onChange={e=>setForm({...form,capability:e.target.value})} className="w-full bg-black border border-zinc-700 rounded-xl p-3 text-white mb-2">{["Driver","General Worker","Cleaner","Retail"].map(c=><option key={c}>{c}</option>)}</select>
      ...
      <div className="bg-[#1a1a1a] rounded-[20px] p-4 mt-4 border border-green-900/30">
        <h2 className="text-white font-bold mb-2">+ Add Job 🔥 PHASE 3</h2>
      ...
      {selectedJob&&<div className="bg-green-400 rounded-[20px] p-4 mt-4"><h2 className="text-black font-black">MATCH: {selectedJob.title} @ {selectedJob.company}</h2><p className="text-black/70 text-xs mb-2">{selectedJob.location} • {selectedJob.pay} — {people.length} candidates</p>{people.map((p:any)=><div key={p.id} className="bg-black rounded-xl p-3 mt-2 flex justify-between items-center"><div><p className="text-white text-sm font-bold">{p.full_name}</p><p className="text-zinc-500 text-xs">{p.phone}</p></div><button onClick={()=>whatsapp(p)} className="bg-green-400 text-black font-bold text-xs px-4 py-2 rounded-full">WhatsApp →</button></div>)}</div>}
    </div>
  )
}
