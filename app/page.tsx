...
export default function Page() {
  ...
  const saveExistingBusinessProfile = async () => {
    if (account?.role !== "business" || !authUser || !currentBusiness) return flash("Your saved Business Profile is not available yet.");
    const businessName = (currentBusiness.name || currentBusiness.business_name || "").trim();
    if (!businessName) return flash("Your business name cannot be empty.");
    const payload = { name: businessName, business_name: businessName, email: currentBusiness.email || "", phone: currentBusiness.phone || "", province: currentBusiness.province || "", town: currentBusiness.town || "", description: currentBusiness.description || "" };
    const { data, error } = await supabase.from("businesses").update(payload).eq("id", currentBusiness.id).eq("auth_user_id", authUser.id).select().single();
    if (error) return flash(error.message);
    setCurrentBusiness(data as Business); setBusinesses(old => old.map(b => b.id === data.id ? data as Business : b));
      ...
      <div className="mx-auto flex min-h-[85vh] max-w-md items-center">
        <Card className="w-full overflow-hidden">
          <div className="bg-emerald-700 p-7 text-white">
            ...
            <div className="mt-5 text-xs font-black uppercase tracking-widest text-orange-300">GUARDIAN WORK V6.8.1</div>
      ...
      <div className="mx-auto flex min-h-[85vh] max-w-md items-center">
        <Card className="w-full overflow-hidden">
          ...
          <div className="p-6">
            ...
            <div className="mt-5 text-center text-xs text-slate-500">V6.8.1 focuses on real actions, secure checkout, verification and honest status.</div>
          </div>
        </Card>
      </div>
    </div>
    ...
    <div className="min-h-screen bg-slate-50 px-4 py-8">
      <div className="mx-auto flex min-h-[85vh] max-w-xl items-center">
        <Card className="w-full p-6 sm:p-8">
          <div className="text-xs font-black uppercase tracking-widest text-orange-500">V6.8.1 setup</div>
    ...
    <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => { setProfileMenuOpen(false); nav("profile"); }} aria-label="Open profile" className="rounded-full focus:outline-none focus:ring-2 focus:ring-orange-400">
            {statusAvatar(currentPerson?.name || "GW", currentPerson?.profile_photo_url || null, activeMyStatusCount, "h-10 w-10")}
          </button>
        </div>
      </div>
    </header>
        ...
        <div className="grid gap-8 lg:grid-cols-[1.4fr_.6fr] lg:items-center">
          <div>
            <div className="mb-3 inline-flex rounded-full bg-orange-400 px-3 py-1 text-xs font-black text-slate-950">
              V6.9.0 - STATUS RINGS + PROFILE CHAT + MATCH ENGINE
            </div>
      ...
      <Card className="overflow-hidden border-emerald-200">
        <div className="bg-gradient-to-r from-emerald-700 to-emerald-600 p-6 text-white">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <div className="text-xs font-black uppercase tracking-widest text-emerald-100">GUARDIAN WORK ID · V6.8.1</div>
  ...
  const renderMatches = () => (
    <div className="space-y-6">
      <div>
        <div className="text-xs font-black uppercase tracking-widest text-orange-500">V6.8.1 Match Engine</div>
  ...
  const renderTimeline = () => {
    ...
    return (
      <div className="mx-auto max-w-3xl space-y-5 pb-8">
        ...
        {(() => { const activeStatuses=guardianStatuses.filter(x=>!x.expires_at||new Date(x.expires_at).getTime()>statusNow); const circleOwners=Array.from(new Map(activeStatuses.map(x=>[x.author_auth_user_id,x])).values()); const countFor=(id:string)=>activeStatuses.filter(x=>x.author_auth_user_id===id).length; return <Card className="border-orange-100 bg-white p-4"><div className="flex items-center justify-between gap-3"><div><div className="text-xs font-black uppercase tracking-widest text-orange-500">STATUS CIRCLES</div><div className="mt-1 text-sm font-black text-slate-900">Active work stories</div></div><span className="text-[10px] font-black text-slate-400">Swipe left / right →</span></div><div className="mt-4 flex gap-4 overflow-x-auto pb-2 snap-x snap-mandatory"><button type="button" onClick={()=>document.getElementById("guardian-status-composer")?.scrollIntoView({behavior:"smooth",block:"center"})} className="w-20 shrink-0 snap-start text-center">{statusAvatar(currentPerson?.name||"You",currentPerson?.profile_photo_url||null,countFor(authUser?.id||""),"h-14 w-14 mx-auto")}<div className="mt-2 truncate text-[10px] font-black text-slate-800">My Status</div></button>{circleOwners.filter(x=>x.author_auth_user_id!==authUser?.id).map(x=>{const name=ownerName(x);const person=x.author_type==="person"?people.find(p=>p.id===x.author_id):null;return <button key={x.author_auth_user_id} type="button" onClick={()=>document.getElementById(`status-${x.id}`)?.scrollIntoView({behavior:"smooth",block:"center"})} className="w-20 shrink-0 snap-start text-center">{statusAvatar(name,person?.profile_photo_url||null,countFor(x.author_auth_user_id),"h-14 w-14 mx-auto")}<div className="mt-2 truncate text-[10px] font-black text-slate-800">{name}</div></button>})}{!circleOwners.length&&<div className="py-4 text-xs text-slate-400">No active statuses yet.</div>}</div></Card>; })()}

        <Card className="border-slate-200 bg-white p-4">
          <div className="flex items-center gap-4">
            <button type="button" onClick={()=>nav("profile")} className="rounded-full focus:outline-none focus:ring-2 focus:ring-orange-400">
              {statusAvatar(currentPerson?.name || "You", currentPerson?.profile_photo_url || null, activeMyStatusCount, "h-16 w-16")}
            </button>
            <div className="min-w-0 flex-1"><div className="text-xs font-black uppercase tracking-widest text-orange-500">MY STATUS</div><div className="mt-1 font-black text-slate-900">{activeMyStatusCount ? `${activeMyStatusCount} active status${activeMyStatusCount===1?"":"es"}` : "Add a status"}</div><div className="text-xs text-slate-500">The orange ring follows the duration you choose. Your status stays in your GUARDIAN timeline forever.</div></div>
            <Button variant="orange" onClick={()=>document.getElementById("guardian-status-composer")?.scrollIntoView({behavior:"smooth",block:"center"})}>{activeMyStatusCount ? "Add another" : "Post"}</Button>
          </div>
        </Card>
        ...
        {sponsoredAds.filter(ad=>ad.status==="active"&&ad.billing_status==="paid").map(ad=>{const business=businesses.find(b=>b.id===ad.business_id);const businessName=business?.name||business?.business_name||"GUARDIAN business";const targetText=[ad.target_town,ad.target_province].filter(Boolean).join(" · ");return <Card key={`sponsored-${ad.id}`} className="overflow-hidden border-orange-200"><div className="flex items-center justify-between gap-3 border-b border-orange-100 bg-orange-50 px-5 py-3"><div className="text-[10px] font-black uppercase tracking-widest text-orange-700">📣 Sponsored · Paid placement</div><div className="text-[10px] font-bold text-slate-400">{targetText||"GUARDIAN WORK community"}</div></div><div className="p-5"><div className="flex items-start gap-3"><div className="grid h-12 w-12 shrink-0 place-items-center rounded-full bg-emerald-100 font-black text-emerald-700">{initials(businessName)}</div><div><div className="font-black text-slate-900">{businessName}</div><div className="text-xs text-slate-500">Sponsored business campaign</div></div></div><h3 className="mt-4 text-xl font-black text-slate-900">{ad.title}</h3>{ad.body&&<p className="mt-2 whitespace-pre-wrap text-sm leading-7 text-slate-700">{ad.body}</p>}{ad.image_url&&<img src={ad.image_url} alt={ad.title} className="mt-4 max-h-[420px] w-full rounded-2xl object-cover"/>}<div className="mt-4 flex flex-wrap items-center gap-2"><Button variant="orange" onClick={()=>openAd(ad)}>{ad.cta_label||"Learn more"}</Button><span className="rounded-xl bg-slate-100 px-3 py-2 text-[10px] font-black text-slate-500">Paid & verified</span></div></div></Card>})}
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <Card className="p-6 border-emerald-200">
          <div className="flex items-start justify-between gap-3"><div><div className="text-xs font-black uppercase tracking-widest text-emerald-600">SAVED BUSINESS IDENTITY</div><h2 className="mt-1 text-xl font-black">{currentBusiness?.business_name || currentBusiness?.name || "Your Business"}</h2><p className="mt-1 text-xs text-slate-500">Created during sign-up. Your Business Profile is already part of your account — there is no second Create Profile step inside the portal.</p></div><span className="rounded-full bg-emerald-50 px-3 py-1 text-[10px] font-black text-emerald-700">PROFILE SAVED</span></div>
          {currentBusiness ? <div className="mt-5 space-y-3">
            {([["name","Business name"],["email","Email"],["phone","Phone"],["province","Province"],["town","Town"]] as const).map(([key,label])=><input key={key} value={(currentBusiness as any)[key] || (key==="name" ? currentBusiness.business_name || "" : "")} onChange={e=>setCurrentBusiness(old=>old ? ({...old,[key]:e.target.value,...(key==="name"?{business_name:e.target.value}:{})} as Business) : old)} placeholder={label} className="w-full rounded-xl border border-slate-200 px-3 py-3 text-sm"/>)}
            <textarea value={currentBusiness.description || ""} onChange={e=>setCurrentBusiness(old=>old ? ({...old,description:e.target.value} as Business) : old)} placeholder="Business description" rows={3} className="w-full rounded-xl border border-slate-200 px-3 py-3 text-sm"/>
            <Button variant="green" onClick={()=>void saveExistingBusinessProfile()}>Save Business Profile</Button>
          </div> : <div className="mt-5 rounded-2xl bg-orange-50 p-4 text-sm font-bold text-orange-700">Your saved Business Profile is not loading. Sign out and sign back in to refresh the account.</div>}
        </Card>
  ...
  const renderAlerts = () => {
    ...
    return (
      <div className="space-y-6">
        <div>
          <div className="text-xs font-black uppercase tracking-widest text-orange-500">V6.8.1 - GUARDIAN ALERTS</div>
    ...
    <div className={`guardian-work ${theme === "dark" ? "guardian-dark" : ""} min-h-screen bg-slate-50 text-slate-900`}>
      ...
      <footer className="border-t border-slate-200 bg-white">
        <div className="mx-auto flex max-w-7xl flex-col gap-2 px-4 py-8 text-xs text-slate-500 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div>V6.9.0 - Status Rings + Profile Chat + Payment Methods</div>
        </div>
      </footer>
