...
export default function Page() {
  ...
  const startPaystackCheckout = async (args: { requestId: string; amountCents: number; supportType?: string; whatsapp?: boolean; kind?: "support" | "whatsapp" | "ad" }) => {
    if (!authUser?.email) {
      flash("Your account needs an email address before card payment can start.");
      return false;
    }
    ...
    try {
      ...
      const accessToken = session.data.session?.access_token;
      if (!accessToken) {
        flash("Your secure session has expired. Please sign in again.");
        return false;
      }
      ...
      if (!res.ok || !body.authorization_url) {
        flash(body.error || "Card checkout could not be started. Check the server payment configuration.");
        return false;
      }
  ...
  const confirmWhatsAppPaymentMethod = async () => {
    ...
    const { data, error } = await supabase
      .from("guardian_whatsapp_payment_requests")
      .insert({ job_seeker_id: currentPerson.id, amount_cents: 1000, payment_method: "credit_card", status: "pending" })
      .select()
      .single();
    if (error) {
      setPaymentProcessing(false);
    ...
    const opened = await startPaystackCheckout({ requestId: data.id, amountCents: 1000, whatsapp: true, kind: "whatsapp" });
        ...
        <div className="grid gap-8 lg:grid-cols-[1.4fr_.6fr] lg:items-center">
          <div>
            <div className="mb-3 inline-flex rounded-full bg-orange-400 px-3 py-1 text-xs font-black text-slate-950">
              V6.8.1 - SUPPORT + MEDIA TIMELINE + MATCH ENGINE
            </div>
  ...
  const renderSupport = () => (
    <div className="space-y-6">
      <div>
        <div className="text-xs font-black uppercase tracking-widest text-orange-500">GUARDIAN SUPPORT · V6.8.1</div>
  ...
  const renderSettings = () => (
    <div className="space-y-6">
      ...
      <div className="text-xs text-slate-400">GUARDIAN WORK V6.8.1 - Support + Media Profiles + Timeline Attachments + Advertising</div>
    </div>
  );
    ...
    <div className={`guardian-work ${theme === "dark" ? "guardian-dark" : ""} min-h-screen bg-slate-50 text-slate-900`}>
      ...
      <footer className="border-t border-slate-200 bg-white">
        <div className="mx-auto flex max-w-7xl flex-col gap-2 px-4 py-8 text-xs text-slate-500 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div>V6.8.1 - Functional Hardening · Verified Actions</div>
        </div>
      </footer>
