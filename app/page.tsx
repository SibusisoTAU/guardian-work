...
export default function Page() {
        ...
        <div className="grid gap-8 lg:grid-cols-[1.4fr_.6fr] lg:items-center">
          <div>
            <div className="mb-3 inline-flex rounded-full bg-orange-400 px-3 py-1 text-xs font-black text-slate-950">
              V6.8.2 - STATUS + PRIVATE CHATS + MATCH ENGINE
            </div>
  ...
  const renderTimeline = () => {
    ...
    return (
      <div className="mx-auto max-w-3xl space-y-5 pb-8">
        ...
        <div className="flex items-center justify-between"><div><div className="text-xs font-black uppercase tracking-widest text-orange-500">STATUS UPDATES</div><h2 className="mt-1 text-xl font-black text-slate-900">Recent work stories</h2></div><span className="rounded-full bg-emerald-50 px-3 py-1 text-[10px] font-black text-emerald-700">{guardianStatuses.length} statuses</span></div>
        ...
        {statuses.map(status=>{
          ...
          return <Card key={status.id} className="overflow-hidden"><div className="p-5">
            <div className="flex items-center gap-3"><div className="grid h-12 w-12 place-items-center rounded-full bg-orange-100 text-xs font-black text-orange-700 ring-2 ring-emerald-200">{initials(name)}</div><div className="min-w-0 flex-1"><div className="font-black text-slate-900">{name}</div><div className="text-xs text-slate-500">{status.author_type === "business" ? "Business" : (people.find(p=>p.id===status.author_id)?.job_title||"Work identity")} · {formatDate(status.created_at)}</div></div><span className="rounded-full bg-emerald-50 px-3 py-1 text-[10px] font-black text-emerald-700">PUBLIC STATUS</span></div>
  ...
  const renderSettings = () => (
    <div className="space-y-6">
      ...
      <div className="text-xs text-slate-400">GUARDIAN WORK V6.8.2 - Status + Private Chats + Payment Methods</div>
    </div>
  );
    ...
    <div className={`guardian-work ${theme === "dark" ? "guardian-dark" : ""} min-h-screen bg-slate-50 text-slate-900`}>
      ...
      <footer className="border-t border-slate-200 bg-white">
        <div className="mx-auto flex max-w-7xl flex-col gap-2 px-4 py-8 text-xs text-slate-500 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div>V6.8.2 - Status + Private Chats + Payment Methods</div>
        </div>
      </footer>
