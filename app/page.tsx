...
export default function Page() {
      ...
      <div className="mx-auto flex min-h-[85vh] max-w-md items-center">
        <Card className="w-full overflow-hidden">
          <div className="bg-emerald-700 p-7 text-white">
            ...
            <div className="mt-5 text-xs font-black uppercase tracking-widest text-orange-300">GUARDIAN WORK V6.6.1</div>
      ...
      <Card className="overflow-hidden border-emerald-200">
        <div className="bg-gradient-to-r from-emerald-700 to-emerald-600 p-6 text-white">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <div className="text-xs font-black uppercase tracking-widest text-emerald-100">GUARDIAN WORK ID · V6.6.1</div>
  ...
  const renderMatches = () => (
    <div className="space-y-6">
      <div>
        <div className="text-xs font-black uppercase tracking-widest text-orange-500">V6.6.1 Match Engine</div>
    ...
    return (
      <div className="fixed inset-0 z-[80] overflow-y-auto bg-slate-950/60 p-4" onClick={() => setSelectedTalent(null)}>
        <div className="mx-auto mt-8 max-w-2xl" onClick={e => e.stopPropagation()}>
          <Card className="overflow-hidden">
            <div className="bg-emerald-700 p-6 text-white"><div className="flex items-start justify-between gap-4"><div className="flex gap-4"><div className="grid h-16 w-16 shrink-0 place-items-center rounded-full bg-orange-100 text-xl font-black text-orange-700">{initials(p.name)}</div><div><div className="text-2xl font-black">{p.name || "GUARDIAN WORK member"}</div><div className="font-bold text-orange-300">{p.job_title || "Work seeker"}</div><div className="mt-1 text-xs text-emerald-100">{[p.town,p.province].filter(Boolean).join(" - ")}</div></div></div><button onClick={()=>setSelectedTalent(null)} className="rounded-xl bg-white/10 px-3 py-2 font-black text-white">Close</button></div></div>
            <div className="space-y-5 p-6">
              {talentProfileBusy ? <div className="rounded-2xl bg-slate-50 p-5 text-sm font-bold text-slate-500">Loading Work Profile...</div> : <>
                ...
                {account?.role === "business" && currentBusiness && <div className="flex flex-wrap gap-2 border-t border-slate-200 pt-5"><Button variant="orange" disabled={contactBusy} onClick={()=>sendContactRequest(p)}>{contactBusy ? "Sending..." : "Contact this person"}</Button><Button variant="light" onClick={()=>shortlistPerson(p)}>Save to shortlist</Button></div>}
              </>}
            </div>
          </Card>
        </div>
  ...
  const renderSettings = () => (
    <div className="space-y-6">
      ...
      <div className="text-xs text-slate-400">GUARDIAN WORK V6.6.1 - No Dead Buttons - Secure Accounts - Smart Discovery - Saved Work</div>
    </div>
  );
