...
type GuardianStatus = { id: string; author_type: "person" | "business"; author_auth_user_id: string; author_id: string | null; content: string | null; created_at: string; };
...
type StatusComment = { id: string; status_id: string; author_auth_user_id: string; author_type: "person" | "business"; author_id: string | null; content: string; visibility: "public" | "private"; created_at: string; };
type DirectMessage = { id: string; sender_auth_user_id: string; recipient_auth_user_id: string; content: string; created_at: string; };
...
export default function Page() {
  ...
  const loadAll = async () => {
    ...
    const [
      ...
      authUser
        ...
        : Promise.resolve({ data: [], error: null }),
      supabase.from("guardian_statuses").select("*").order("created_at", { ascending: false }).limit(100),
  ...
  const createStatus = async () => {
    ...
    if (!authorId) return flash("Complete your GUARDIAN WORK profile first.");
    setStatusBusy(true);
    const { data, error } = await supabase.from("guardian_statuses").insert({ author_type: isBusinessAuthor ? "business" : "person", author_auth_user_id: authUser.id, author_id: authorId, content: postText.trim() || null, visibility: "public" }).select().single();
    ...
    setPostText(""); setStatusFiles([]); setStatusBusy(false);
  ...
  const addStatusComment = async (status: GuardianStatus) => {
    ...
    const isBusinessAuthor = account?.role === "business" && !!currentBusiness;
    const authorId = isBusinessAuthor ? currentBusiness?.id : currentPerson?.id;
    if (!authorId) return flash("Complete your profile first.");
    setStatusCommentBusy(old => ({ ...old, [status.id]: true }));
    const visibility = statusCommentVisibility[status.id] || "public";
    const { data, error } = await supabase.from("guardian_status_comments").insert({ status_id: status.id, author_auth_user_id: authUser.id, author_type: isBusinessAuthor ? "business" : "person", author_id: authorId, content: content || "", visibility }).select().single();
  ...
  const loadChatMessages = async (partnerAuthId: string) => {
    ...
    if (error) return flash(error.message);
    setChatMessages((data || []) as DirectMessage[]);
  };

  const openPrivateChatFromProfile = async (partnerAuthId: string) => {
    if (!partnerAuthId || partnerAuthId === authUser?.id) return;
    await loadChatMessages(partnerAuthId);
  };

  const sendDirectMessage = async () => {
    if (!authUser || !chatPartnerAuthId || !chatDraft.trim()) return;
    setChatBusy(true);
  ...
  const activeMyStatusCount = guardianStatuses.filter((s) => {
    ...
    return statusNow - new Date(s.created_at).getTime() < 24 * 60 * 60 * 1000;
  }).length;
  ...
  const openStatusOwnerProfile = async (status: GuardianStatus) => {
    if (status.author_type !== "person" || !status.author_id) return flash("Business profiles will be connected to private chat next.");
    const person = people.find((p) => p.id === status.author_id);
    if (!person) return flash("That profile is not available right now.");
    await openTalentProfile(person);
  };
        ...
        <div className="grid gap-8 lg:grid-cols-[1.4fr_.6fr] lg:items-center">
          <div>
            <div className="mb-3 inline-flex rounded-full bg-orange-400 px-3 py-1 text-xs font-black text-slate-950">
              V6.8.3 - STATUS RINGS + PROFILE CHAT + MATCH ENGINE
            </div>
  ...
  const renderChats = () => {
    const filteredPeople = people.filter(p =>
      p.auth_user_id && p.auth_user_id !== authUser?.id &&
      (safeText(p.name).toLowerCase().includes(chatSearch.toLowerCase()) ||
       safeText(p.job_title).toLowerCase().includes(chatSearch.toLowerCase()) ||
       safeText(p.town).toLowerCase().includes(chatSearch.toLowerCase()))
    ).slice(0, 30);
    const partner = people.find(p => p.auth_user_id === chatPartnerAuthId);
    return (
      <div className="mx-auto max-w-5xl overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-200 bg-emerald-700 p-5 text-white">
          <div className="text-xs font-black uppercase tracking-[0.2em] text-orange-300">PRIVATE WORK CHAT</div>
          <h1 className="mt-1 text-2xl font-black">Talk to people one-to-one.</h1>
          <p className="mt-1 max-w-2xl text-sm text-emerald-50">Private conversations are only between you and the person you choose. They are separate from public Status.</p>
        </div>
        <div className="grid min-h-[560px] md:grid-cols-[280px_1fr]">
          <aside className="border-b border-slate-200 bg-slate-50 md:border-b-0 md:border-r">
            <div className="p-3"><input value={chatSearch} onChange={e=>setChatSearch(e.target.value)} placeholder="Search people..." className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-xs outline-none" /></div>
            <div className="max-h-[480px] overflow-y-auto p-2">
              {filteredPeople.map(person => (
                <button key={person.id} onClick={()=>loadChatMessages(person.auth_user_id!)} className={`flex w-full items-center gap-3 rounded-2xl p-3 text-left ${chatPartnerAuthId===person.auth_user_id ? "bg-emerald-100" : "hover:bg-white"}`}>
                  <div className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-orange-100 text-xs font-black text-orange-700">{initials(person.name)}</div>
                  <div className="min-w-0"><div className="truncate text-sm font-black text-slate-900">{person.name || "GUARDIAN member"}</div><div className="truncate text-[10px] text-slate-500">{person.job_title || "Work identity"} · {person.town || ""}</div></div>
                </button>
              ))}
              {!filteredPeople.length && <div className="p-4 text-xs text-slate-500">No other discoverable people found.</div>}
            </div>
          </aside>
            ...
            {partner ? (
              <>
                <div className="flex items-center gap-3 border-b border-slate-200 p-4">
                  <div className="grid h-10 w-10 place-items-center rounded-full bg-orange-100 text-xs font-black text-orange-700">{initials(partner.name)}</div>
                  <div><div className="font-black text-slate-900">{partner.name}</div><div className="text-xs text-slate-500">{partner.job_title || "Work identity"}</div></div>
                  <span className="ml-auto rounded-full bg-slate-100 px-3 py-1 text-[10px] font-black text-slate-500">PRIVATE</span>
                </div>
                <div className="flex-1 space-y-2 overflow-y-auto bg-slate-50 p-4">
                  {chatMessages.map(m => (
                    <div key={m.id} className={`flex ${m.sender_auth_user_id===authUser?.id ? "justify-end" : "justify-start"}`}>
                      <div className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm ${m.sender_auth_user_id===authUser?.id ? "bg-emerald-600 text-white" : "bg-white text-slate-700 shadow-sm"}`}>
                        <div className="whitespace-pre-wrap">{m.content}</div>
                        <div className={`mt-1 text-[9px] ${m.sender_auth_user_id===authUser?.id ? "text-emerald-100" : "text-slate-400"}`}>{new Date(m.created_at).toLocaleTimeString("en-ZA",{hour:"2-digit",minute:"2-digit"})}</div>
                      </div>
                    </div>
                  ))}
                  {!chatMessages.length && <div className="grid h-full place-items-center text-center text-sm text-slate-400"><div><div className="text-3xl">💬</div><div className="mt-2 font-black">Start the private work conversation.</div></div></div>}
                </div>
                <div className="border-t border-slate-200 bg-white p-3">
                  <div className="flex gap-2"><input value={chatDraft} onChange={e=>setChatDraft(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();void sendDirectMessage();}}} placeholder="Write a private message..." className="min-w-0 flex-1 rounded-xl border border-slate-200 px-3 py-3 text-sm outline-none" /><Button variant="orange" disabled={chatBusy||!chatDraft.trim()} onClick={sendDirectMessage}>{chatBusy?"Sending...":"Send"}</Button></div>
                  <div className="mt-2 text-[10px] font-bold text-slate-400">Only you and {partner.name || "this person"} can see this chat.</div>
                </div>
              </>
            ) : <div className="grid flex-1 place-items-center p-8 text-center"><div><div className="text-5xl">💬</div><h2 className="mt-3 text-xl font-black text-slate-900">Choose a person</h2><p className="mt-1 max-w-sm text-sm text-slate-500">Select someone on the left to start a private conversation.</p></div></div>}
          </section>
        </div>
      </div>
    );
  ...
  const renderTimeline = () => {
    ...
    const ownerName = (status: GuardianStatus) => status.author_type === "business" ? (businesses.find(b=>b.id===status.author_id)?.name || "GUARDIAN business") : (people.find(p=>p.id===status.author_id)?.name || "GUARDIAN member");
        ...
        {statuses.map(status=>{
          ...
          return <Card key={status.id} className="overflow-hidden"><div className="p-5">
            <div className="flex items-center gap-3"><button type="button" onClick={()=>void openStatusOwnerProfile(status)} className="shrink-0 rounded-full focus:outline-none focus:ring-2 focus:ring-orange-400" aria-label={`Open ${name} profile`}>
              {status.author_type === "person" ? (()=>{const owner=people.find(p=>p.id===status.author_id); const activeCount=guardianStatuses.filter(x=>x.author_auth_user_id===status.author_auth_user_id && statusNow-new Date(x.created_at).getTime()<24*60*60*1000).length; return statusAvatar(name, owner?.profile_photo_url || null, activeCount, "h-12 w-12");})() : statusAvatar(name, null, 0, "h-12 w-12")}
            </button><div className="min-w-0 flex-1"><button type="button" onClick={()=>void openStatusOwnerProfile(status)} className="text-left font-black text-slate-900 hover:text-orange-600">{name}</button><div className="text-xs text-slate-500">{status.author_type === "business" ? "Business" : (people.find(p=>p.id===status.author_id)?.job_title||"Work identity")} · {formatDate(status.created_at)}</div></div><span className="rounded-full bg-emerald-50 px-3 py-1 text-[10px] font-black text-emerald-700">PUBLIC STATUS</span></div>
  ...
  const renderSettings = () => (
    <div className="space-y-6">
      ...
      <div className="text-xs text-slate-400">GUARDIAN WORK V6.8.3 - Status Rings + Profile Chat + Payment Methods</div>
    </div>
  );
    ...
    <div className={`guardian-work ${theme === "dark" ? "guardian-dark" : ""} min-h-screen bg-slate-50 text-slate-900`}>
      ...
      <footer className="border-t border-slate-200 bg-white">
        <div className="mx-auto flex max-w-7xl flex-col gap-2 px-4 py-8 text-xs text-slate-500 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div>V6.8.3 - Status Rings + Profile Chat + Payment Methods</div>
        </div>
      </footer>
