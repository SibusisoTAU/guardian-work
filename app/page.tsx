...
export default function Page() {
  ...
  const [adForm, setAdForm] = useState({
    ...
    target_province: "", target_town: "", target_skills: "", starts_at: "", ends_at: "",
  ...
  const loadAll = async () => {
    ...
    if (!adImpressionsRes.error) setAdImpressions((adImpressionsRes.data || []) as AdImpression[]);
  ...
  useEffect(() => {
    if (!currentPerson) return;
    const channel = supabase.channel("guardian-work-live", {
      config: { presence: { key: currentPerson.id } },
    });
    ...
    channel
      ...
      .subscribe(async (status) => {
        ...
        if (status === "SUBSCRIBED") {
          await channel.track({ person_id: currentPerson.id, online_at: new Date().toISOString() });
        }
      });
    return () => {
  ...
  const smartMatches = useMemo<SmartMatch[]>(() => {
    if (!currentPerson) return [];
    const personSkills = safeText(currentPerson.skills).toLowerCase().split(/[,;|]/).map(s => s.trim()).filter(Boolean);
    const personTitle = safeText(currentPerson.job_title).toLowerCase();
    const personTown = safeText(currentPerson.town).toLowerCase();
    const personProvince = safeText(currentPerson.province).toLowerCase();
    const personExperience = safeText(currentPerson.experience).toLowerCase();

    return opportunities.map((o) => {
      const required = safeText(o.skills_required).toLowerCase().split(/[,;|]/).map(s => s.trim()).filter(Boolean);
      const matchedSkills = required.filter(skill => personSkills.some(ps => ps.includes(skill) || skill.includes(ps)));
      const skillScore = required.length ? Math.min(100, Math.round((matchedSkills.length / required.length) * 100)) : (personTitle && safeText(o.title).toLowerCase().includes(personTitle) ? 80 : 50);
      const locationScore = personTown && safeText(o.town).toLowerCase() === personTown ? 100 : personProvince && safeText(o.province).toLowerCase() === personProvince ? 75 : 25;
      const titleScore = personTitle && (safeText(o.title).toLowerCase().includes(personTitle) || personTitle.includes(safeText(o.title).toLowerCase())) ? 100 : 35;
      const expScore = safeText(o.experience_required).trim() ? (personExperience ? 80 : 35) : 100;
      const availabilityScore = safeText(currentPerson.availability).toLowerCase().includes("available") ? 100 : 55;
      const score = Math.max(0, Math.min(100, Math.round(skillScore * .35 + expScore * .20 + locationScore * .20 + availabilityScore * .15 + titleScore * .10)));
      const reasons: string[] = [];
      if (matchedSkills.length) reasons.push(`${matchedSkills.length} skill${matchedSkills.length > 1 ? "s" : ""} match`);
      if (locationScore >= 75) reasons.push("location match");
      if (titleScore >= 80) reasons.push("work identity match");
      if (availabilityScore >= 90) reasons.push("availability match");
      return { opportunity: o, score, reasons: reasons.length ? reasons : ["potential opportunity"] };
  ...
  const businessFilteredPeople = useMemo(() => {
    ...
    return people.filter((p) => {
      ...
      const haystack = [p.name, p.job_title, p.town, p.province, p.skills, p.experience]
        .map(safeText).join(" ").toLowerCase();
      const skills = safeText(p.skills).toLowerCase();
      return (!q || haystack.includes(q)) &&
        (!businessTalentProvince || p.province === businessTalentProvince) &&
        (!businessTalentTown || safeText(p.town).toLowerCase().includes(businessTalentTown.toLowerCase())) &&
        (!skillQ || skills.includes(skillQ));
    });
  }, [people, businessTalentSearch, businessTalentProvince, businessTalentTown, businessTalentSkills]);

  const targetedAds = useMemo(() => {
    if (!currentPerson) return [];
    const province = safeText(currentPerson.province).toLowerCase();
    const town = safeText(currentPerson.town).toLowerCase();
    const skills = safeText(currentPerson.skills).toLowerCase().split(/[,;|]/).map(x => x.trim()).filter(Boolean);
    return sponsoredAds
      .filter(ad => {
        const startOk = !ad.starts_at || new Date(ad.starts_at).getTime() <= Date.now();
        const endOk = !ad.ends_at || new Date(ad.ends_at).getTime() >= Date.now();
        if (!startOk || !endOk || ad.status !== "active") return false;
        const targetProvince = safeText(ad.target_province).toLowerCase();
        const targetTown = safeText(ad.target_town).toLowerCase();
        const targetSkills = safeText(ad.target_skills).toLowerCase().split(/[,;|]/).map(x => x.trim()).filter(Boolean);
        const provinceOk = !targetProvince || targetProvince === province;
        const townOk = !targetTown || town.includes(targetTown);
        const skillOk = !targetSkills.length || targetSkills.some(ts => skills.some(ps => ps.includes(ts) || ts.includes(ps)));
        const impressionsToday = adImpressions.filter(i => i.ad_id === ad.id && i.event_type === "impression").length;
        return provinceOk && townOk && skillOk && impressionsToday < (ad.daily_frequency_cap || 2);
      })
      .sort((a,b) => {
        const score = (ad: SponsoredAd) => {
          let n = 0;
          if (ad.target_province && safeText(ad.target_province).toLowerCase() === province) n += 3;
          if (ad.target_town && town.includes(safeText(ad.target_town).toLowerCase())) n += 2;
          if (ad.target_skills) n += 1;
          return n;
        };
        return score(b) - score(a);
      })
      .slice(0, 2);
  }, [currentPerson, sponsoredAds, adImpressions]);

  const adminProvinceStats = useMemo(() => {
    const map = new Map<string, number>();
    people.filter(p => p.profile_visibility !== "hidden").forEach(p => {
      const key = p.province || "Not specified";
      map.set(key, (map.get(key) || 0) + 1);
    });
    return Array.from(map.entries()).sort((a,b) => b[1]-a[1]);
  }, [people]);

  const adminJobStats = useMemo(() => {
    const map = new Map<string, number>();
    people.filter(p => p.profile_visibility !== "hidden").forEach(p => {
      const key = p.job_title || "Work seeker";
      map.set(key, (map.get(key) || 0) + 1);
    });
    return Array.from(map.entries()).sort((a,b) => b[1]-a[1]).slice(0,8);
  }, [people]);

  const adPerformance = useMemo(() => sponsoredAds.map(ad => ({
    ...ad,
    impressions: adImpressions.filter(i => i.ad_id === ad.id && i.event_type === "impression").length,
    clicks: adImpressions.filter(i => i.ad_id === ad.id && i.event_type === "click").length,
  })), [sponsoredAds, adImpressions]);
  ...
  const refreshMatches = async () => {
    ...
    flash("Smart Match refreshed.");
  };

  const activateWhatsAppPlan = async () => {
    if (!currentPerson) return;
    await saveNotificationPreferences({ whatsapp_enabled: true, whatsapp_opt_in: true, whatsapp_subscription_status: "pending" });
    flash("WhatsApp Alerts selected. Payment integration can now activate the R10/month subscription.");
  };

  const recordAdEvent = async (ad: SponsoredAd, event_type: "impression" | "click") => {
    if (!currentPerson) return;
    if (event_type === "impression" && adImpressions.some(i => i.ad_id === ad.id && i.event_type === "impression")) return;
    const { data, error } = await supabase.from("ad_impressions").insert({ ad_id: ad.id, job_seeker_id: currentPerson.id, event_type }).select().single();
    if (!error && data) setAdImpressions(old => [...old, data as AdImpression]);
  };

  useEffect(() => {
    if (active !== "timeline" || !currentPerson || !targetedAds.length) return;
    targetedAds.forEach(ad => { void recordAdEvent(ad, "impression"); });
  }, [active, currentPerson?.id, targetedAds.map(a => a.id).join(",")]);
  ...
  const createSponsoredAd = async () => {
    ...
    const { data, error } = await supabase.from("sponsored_ads").insert({
      ...adForm, daily_frequency_cap: Math.max(1, Number(adForm.daily_frequency_cap) || 2),
      status: "active", starts_at: adForm.starts_at || null, ends_at: adForm.ends_at || null,
    }).select().single();
    if (error) return flash(error.message);
    setSponsoredAds(old => [data as SponsoredAd, ...old]);
    setAdForm({ title:"", body:"", image_url:"", cta_label:"Learn more", cta_url:"", target_province:"", target_town:"", target_skills:"", starts_at:"", ends_at:"", daily_frequency_cap:2 });
    flash("Sponsored ad created.");
  };
    ...
    <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="hidden rounded-full bg-emerald-50 px-2.5 py-1 text-[10px] font-black text-emerald-700 sm:block">鈼� {liveUsers} live</div>
    ...
    <div className="space-y-6">
        ...
        <div className="grid gap-8 lg:grid-cols-[1.4fr_.6fr] lg:items-center">
          <div>
            <div className="mb-3 inline-flex rounded-full bg-orange-400 px-3 py-1 text-xs font-black text-slate-950">
              V5.7 鈥� LIVE INTELLIGENCE + SMART DISCOVERY
            </div>
      ...
      <Card className="p-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div><div className="text-xs font-black uppercase tracking-widest text-orange-500">V5.6 intelligence</div><h2 className="mt-1 text-xl font-black text-slate-900">馃幆 Your Smart Matches</h2><p className="mt-1 text-xs text-slate-500">Ranked from your skills, experience, location, availability and Work Identity.</p></div>
        ...
        <div className="mt-5 grid gap-3 lg:grid-cols-2">
          {smartMatches.filter(m => !matchDismissed.includes(m.opportunity.id)).slice(0,4).map(m => (
            <div key={m.opportunity.id} className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
              <div className="flex items-start justify-between gap-3"><div><div className="font-black text-slate-900">{m.opportunity.title || "Opportunity"}</div><div className="mt-1 text-xs text-slate-500">{[m.opportunity.town, m.opportunity.province].filter(Boolean).join(" 鈥� ") || "Location flexible"}</div></div><span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-black text-emerald-800">{m.score}% Match</span></div>
              <div className="mt-3 flex flex-wrap gap-1.5">{m.reasons.map(r => <span key={r} className="rounded-full bg-white px-2 py-1 text-[10px] font-bold text-slate-600">鉁� {r}</span>)}</div>
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {posts.map((post, index) => (
          ...
          {index % 3 === 2 && targetedAds[index % Math.max(1, targetedAds.length)] && (() => {
            const ad = targetedAds[index % targetedAds.length];
            return (
              <Card className="mt-4 overflow-hidden border-orange-200">
                <div className="flex items-center justify-between border-b border-orange-100 bg-orange-50 px-4 py-2">
                  <span className="text-[10px] font-black uppercase tracking-widest text-orange-700">Sponsored</span>
                  <span className="text-[10px] font-bold text-slate-400">GUARDIAN WORK partner</span>
                </div>
                <div className="p-5">
                  {ad.image_url && <img src={ad.image_url} alt="" className="mb-4 h-36 w-full rounded-xl object-cover" />}
                  <div className="text-lg font-black text-slate-900">{ad.title}</div>
                  {ad.body && <p className="mt-2 text-sm leading-6 text-slate-600">{ad.body}</p>}
                  <a href={ad.cta_url || "#"} target="_blank" rel="noreferrer" onClick={() => void recordAdEvent(ad, "click")} className="mt-4 inline-flex rounded-xl bg-orange-500 px-4 py-2.5 text-sm font-bold text-white hover:bg-orange-600">{ad.cta_label || "Learn more"}</a>
                </div>
              </Card>
            );
          })()}
    ...
    <div className="space-y-6">
      ...
      <Card className="p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div><div className="text-xs font-black uppercase tracking-widest text-emerald-600">Talent Search</div><h2 className="mt-1 text-xl font-black text-slate-900">Find job seekers where the work is.</h2><p className="mt-1 text-xs text-slate-500">Search discoverable people using your required location and ability.</p></div>
          {currentBusiness && <Button variant="light" onClick={() => { setBusinessTalentProvince(currentBusiness.province || ""); setBusinessTalentTown(currentBusiness.town || ""); }}>Use business location</Button>}
        </div>
        <div className="mt-5 grid gap-3 md:grid-cols-2 lg:grid-cols-4">
          <input value={businessTalentSearch} onChange={e => setBusinessTalentSearch(e.target.value)} placeholder="Name / job title" className="rounded-xl border border-slate-200 px-3 py-3 text-sm" />
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3"><div className="text-xs font-bold text-slate-500">{businessFilteredPeople.length} discoverable person{businessFilteredPeople.length === 1 ? "" : "s"} found</div><Button variant="light" onClick={() => { setBusinessTalentSearch(""); setBusinessTalentProvince(""); setBusinessTalentTown(""); setBusinessTalentSkills(""); }}>Clear search</Button></div>
        <div className="mt-5 grid gap-3 md:grid-cols-2 lg:grid-cols-3">
          {businessFilteredPeople.slice(0, 12).map(p => (
            <div key={p.id} className="rounded-2xl border border-slate-200 p-4">
              <div className="flex gap-3"><div className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-emerald-100 font-black text-emerald-700">{initials(p.name)}</div><div className="min-w-0"><div className="font-black text-slate-900">{p.name || "Work seeker"}</div><div className="text-xs font-bold text-orange-600">{p.job_title || "Work identity"}</div><div className="text-[11px] text-slate-500">{[p.town, p.province].filter(Boolean).join(" 鈥� ") || "Location not specified"}</div></div></div>
              {p.skills && <div className="mt-3 text-xs text-slate-600">{p.skills}</div>}
              <div className="mt-3 flex items-center justify-between gap-2"><span className="rounded-full bg-emerald-50 px-2 py-1 text-[10px] font-black text-emerald-700">Discoverable</span>{p.phone && <a href={`https://wa.me/${p.phone.replace(/\D/g, "")}`} target="_blank" rel="noreferrer" className="rounded-xl bg-orange-500 px-3 py-2 text-[10px] font-black text-white">Contact</a>}</div>
            </div>
          ))}
          {!businessFilteredPeople.length && <div className="rounded-2xl bg-slate-50 p-6 text-sm text-slate-500 md:col-span-2 lg:col-span-3">No discoverable people match those requirements yet.</div>}
        </div>
      </Card>
  ...
  const renderOperations = () => {
    ...
    return (
      <div className="space-y-6">
        <div>
          <div className="text-xs font-black uppercase tracking-widest text-orange-500">V5.7 鈥� Platform Intelligence</div>
          <h1 className="mt-1 text-3xl font-black text-slate-900">See the work ecosystem.</h1>
          <p className="mt-2 text-sm text-slate-500">Live presence, aggregated workforce signals, application flow and sponsored discovery 鈥� based on platform data.</p>
        </div>

        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-6">
          {[
            ["People", people.length], ["Live now", liveUsers], ["Opportunities", opportunities.length],
            ["Applications", applications.length], ["Shortlisted", shortlist], ["Selected", selected],
          ].map(([x, y]) => (
            <Card key={String(x)} className="p-5">
              ...
              {x === "Live now" && <div className="mt-1 text-[10px] font-bold text-emerald-700">鈼� {realtimeStatus}</div>}
            </Card>
          ))}
        </div>
        ...
        <div className="grid gap-5 lg:grid-cols-2">
          <Card className="p-6">
            <div className="font-black text-slate-900">Workforce by province</div>
            <div className="mt-4 space-y-3">
              {adminProvinceStats.map(([province, count]) => (
                <div key={province}>
                  <div className="flex justify-between text-xs font-bold text-slate-600"><span>{province}</span><span>{count}</span></div>
                  <div className="mt-1 h-2 rounded-full bg-slate-100"><div className="h-2 rounded-full bg-emerald-600" style={{width:`${people.length ? count / people.length * 100 : 0}%`}} /></div>
                </div>
              ))}
              {!adminProvinceStats.length && <p className="text-sm text-slate-500">No discoverable workforce data yet.</p>}
            </div>
          </Card>

          <Card className="p-6">
            <div className="font-black text-slate-900">Top work identities</div>
            <div className="mt-4 grid gap-2 sm:grid-cols-2">
              {adminJobStats.map(([job, count]) => <div key={job} className="rounded-xl bg-slate-50 p-3"><div className="text-xs font-bold text-slate-500">{job}</div><div className="mt-1 text-xl font-black text-slate-900">{count}</div></div>)}
              {!adminJobStats.length && <p className="text-sm text-slate-500">No work identities yet.</p>}
            </div>
          </Card>
        </div>
        ...
        <Card className="p-6">
          ...
          <div className="mt-5 space-y-3">
            {[
              ["Applied", applications.filter((a) => a.status === "applied").length], ["Reviewing", reviewing],
              ["Shortlisted", shortlist], ["Interview", interviews], ["Selected", selected],
              ["Not selected", applications.filter((a) => a.status === "not_selected").length],
            ].map(([x, y]) => (
              <div key={String(x)}><div className="flex justify-between text-xs font-bold text-slate-600"><span>{x}</span><span>{y}</span></div><div className="mt-1 h-2 rounded-full bg-slate-100"><div className="h-2 rounded-full bg-emerald-600" style={{width:`${applications.length ? Number(y)/applications.length*100 : 0}%`}} /></div></div>
            ))}
          </div>
        </Card>

        <Card className="p-6">
          <div className="flex items-start justify-between gap-3"><div><div className="text-xs font-black uppercase tracking-widest text-orange-500">Smart timeline ads</div><h2 className="mt-1 text-xl font-black text-slate-900">Create a Sponsored discovery card</h2><p className="mt-1 text-xs text-slate-500">Ads are clearly labelled, targeted by work/location and frequency-capped.</p></div><span className="rounded-full bg-orange-50 px-3 py-1 text-xs font-black text-orange-700">V5.7</span></div>
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            {[
              ["title","Ad title"],["body","Message"],["image_url","Image URL"],["cta_url","Destination URL"],
              ["target_town","Target town / area"],["target_skills","Target skills"],["starts_at","Starts at"],["ends_at","Ends at"],
            ].map(([key,label]) => <input key={key} type={key.includes("_at") ? "datetime-local" : "text"} value={(adForm as any)[key]} onChange={e=>setAdForm({...adForm,[key]:e.target.value})} placeholder={label} className="w-full rounded-xl border border-slate-200 px-3 py-3 text-sm" />)}
            <select value={adForm.target_province} onChange={e=>setAdForm({...adForm,target_province:e.target.value})} className="w-full rounded-xl border border-slate-200 px-3 py-3 text-sm"><option value="">All provinces</option>{provinces.map(p=><option key={p}>{p}</option>)}</select>
            <input type="number" min="1" max="20" value={adForm.daily_frequency_cap} onChange={e=>setAdForm({...adForm,daily_frequency_cap:Number(e.target.value)})} placeholder="Daily frequency cap" className="w-full rounded-xl border border-slate-200 px-3 py-3 text-sm" />
          </div>
          <Button className="mt-4" variant="orange" onClick={createSponsoredAd}>Publish Sponsored Ad</Button>
          <div className="mt-6 space-y-2">
            {adPerformance.map(ad=><div key={ad.id} className="flex flex-col gap-2 rounded-xl border border-slate-200 p-4 sm:flex-row sm:items-center sm:justify-between"><div><div className="font-black text-slate-900">{ad.title}</div><div className="text-[10px] font-bold uppercase text-orange-600">Sponsored 鈥� {ad.status}</div></div><div className="text-xs font-bold text-slate-500">{ad.impressions} impressions 鈥� {ad.clicks} clicks</div></div>)}
            {!adPerformance.length && <p className="text-sm text-slate-500">No sponsored ads created yet.</p>}
          </div>
        </Card>
      </div>
    );
  ...
  const renderAlerts = () => {
    ...
    return (
      <div className="space-y-6">
        <div>
          <div className="text-xs font-black uppercase tracking-widest text-orange-500">V5.6 鈥� GUARDIAN ALERTS</div>
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          <Card className={`p-5 ${pref?.portal_enabled !== false ? "ring-2 ring-emerald-100" : ""}`}>
            <div className="text-2xl">馃煝</div><h2 className="mt-3 font-black">GUARDIAN Portal</h2>
          ...
          <Card className={`p-5 ${pref?.whatsapp_enabled ? "ring-2 ring-orange-200" : ""}`}>
            <div className="text-2xl">馃摬</div><h2 className="mt-3 font-black">WhatsApp Alerts</h2>
            <p className="mt-1 text-sm text-slate-500">External delivery channel 鈥� <b>R10/month</b> per job seeker.</p>
            ...
            <Button className="mt-4 w-full" variant="orange" onClick={activateWhatsAppPlan}>Choose WhatsApp 鈥� R10/month</Button>
          </Card>
          <Card className="p-5">
            <div className="text-2xl">馃敟</div><h2 className="mt-3 font-black">Both</h2>
  ...
  const renderSettings = () => (
    <div className="space-y-6">
      <div><div className="text-xs font-black uppercase tracking-widest text-orange-500">Settings</div><h1 className="mt-1 text-3xl font-black text-slate-900">Your GUARDIAN WORK controls.</h1></div>
      <div className="grid gap-5 lg:grid-cols-2">
        <Card className="p-6">
          <div className="flex items-center justify-between gap-4 border-b border-slate-100 pb-5"><div><div className="font-black text-slate-900">Profile visibility</div><div className="text-xs text-slate-500">Control whether your Work Identity can be discovered.</div></div><span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-black text-emerald-700">{currentPerson?.profile_visibility || "discoverable"}</span></div>
          <div className="pt-5 text-sm text-slate-600">Your discoverability settings help businesses find people by ability, location and work identity.</div>
        </Card>
        <Card className="p-6">
          <div className="font-black text-slate-900">Language</div>
          <div className="mt-1 text-xs text-slate-500">GUARDIAN WORK interface language.</div>
          <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-bold text-slate-700">English (EN)</div>
        </Card>
        <Card className="p-6">
          <div className="font-black text-slate-900">Notifications</div>
          <div className="mt-1 text-xs text-slate-500">Manage Portal and WhatsApp Alert preferences.</div>
          <Button className="mt-4" variant="orange" onClick={() => nav("alerts")}>Manage Alerts</Button>
        </Card>
        <Card className="p-6">
          <div className="font-black text-slate-900">GUARDIAN WORK</div>
          <div className="mt-1 text-xs text-slate-500">Make your ability discoverable.</div>
          <div className="mt-4 rounded-xl bg-emerald-50 px-4 py-3 text-xs font-black text-emerald-700">V5.7 鈥� Live Intelligence, Smart Discovery & Alerts</div>
        </Card>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <footer className="border-t border-slate-200 bg-white">
        <div className="mx-auto flex max-w-7xl flex-col gap-2 px-4 py-8 text-xs text-slate-500 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div>V5.7 鈥� Live Intelligence, Smart Discovery & Alerts</div>
        </div>
      </footer>
