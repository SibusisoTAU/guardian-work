...
export default function Page() {
  ...
  const createOpportunity = async () => {
    if (!currentBusiness) return flash("Your saved business profile could not be loaded. Please refresh or sign in again.");
  ...
  const myStatusName = currentBusiness?.business_name || currentBusiness?.name || currentPerson?.name || (account?.role === "admin" ? "GUARDIAN WORK Admin" : "GUARDIAN WORK");
  const myStatusPhoto = currentPerson?.profile_photo_url || null;
      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => { setProfileMenuOpen(false); nav("profile"); }} aria-label="Open profile" className="rounded-full focus:outline-none focus:ring-2 focus:ring-orange-400">
            {statusAvatar(myStatusName, myStatusPhoto, activeMyStatusCount, "h-10 w-10")}
          </button>
        </div>
      </div>
    </header>
  ...
  const renderTimeline = () => {
    ...
    const statusCircleItems = Array.from(new Map(activeStatuses.map(s => [s.author_auth_user_id, s])).values());
    const publicSponsoredAds = sponsoredAds.filter(ad => {
      const now = Date.now();
      if (ad.status !== "active") return false;
      if (ad.starts_at && new Date(ad.starts_at).getTime() > now) return false;
      if (ad.ends_at && new Date(ad.ends_at).getTime() < now) return false;
    ...
    const ownerName = (status: GuardianStatus) => status.author_type === "business" ? (businesses.find(b=>b.id===status.author_id)?.name || businesses.find(b=>b.id===status.author_id)?.business_name || "GUARDIAN business") : status.author_type === "admin" ? "GUARDIAN WORK Admin" : (people.find(p=>p.id===status.author_id)?.name || "GUARDIAN member");
    ...
    return (
      <div className="mx-auto max-w-3xl space-y-5 pb-8">
        ...
        <Card className="border-slate-200 bg-white p-4">
          <div className="flex items-center gap-4">
            <button type="button" onClick={()=>nav("profile")} className="rounded-full focus:outline-none focus:ring-2 focus:ring-orange-400">
              {statusAvatar(myStatusName, myStatusPhoto, activeMyStatusCount, "h-16 w-16")}
            </button>
        ...
        <div className="rounded-3xl border border-orange-100 bg-white p-4 shadow-sm">
          <div className="flex items-center justify-between gap-3"><div><div className="text-[10px] font-black uppercase tracking-widest text-orange-500">ACTIVE STATUS</div><div className="mt-1 text-sm font-black text-slate-900">What's happening in the GUARDIAN WORK community</div></div><span className="text-[10px] font-bold text-slate-400">Swipe →</span></div>
          <div className="mt-4 flex gap-4 overflow-x-auto pb-2">
            <button type="button" onClick={()=>nav("status")} className="w-20 shrink-0 text-center">{statusAvatar(currentPerson?.name || currentBusiness?.name || "GW", currentPerson?.profile_photo_url || null, activeMyStatusCount, "h-14 w-14") }<div className="mt-2 truncate text-[10px] font-black text-slate-700">My Status</div></button>
            {statusCircleItems.map(status => {
              const person = status.author_type === "person" ? people.find(p=>p.id===status.author_id) : null;
              const business = status.author_type === "business" ? businesses.find(b=>b.id===status.author_id) : null;
              const name = ownerName(status);
              const count = activeStatuses.filter(x=>x.author_auth_user_id===status.author_auth_user_id).length;
              const photo = person?.profile_photo_url || null;
              return <button key={status.author_auth_user_id} type="button" onClick={()=>void openStatusOwnerProfile(status)} className="w-20 shrink-0 text-center">{statusAvatar(name, photo, count, "h-14 w-14") }<div className="mt-2 truncate text-[10px] font-black text-slate-700">{name}</div><div className="truncate text-[9px] font-bold text-orange-600">{business ? "Business" : status.author_type === "admin" ? "Admin" : "Status"}</div></button>;
            })}
          </div>
        </div>
        ...
        {publicSponsoredAds.map(ad => <Card key={`sponsored-${ad.id}`} className="overflow-hidden border-orange-200 bg-orange-50/40"><div className="p-5"><div className="flex items-center justify-between gap-3"><div className="text-[10px] font-black uppercase tracking-widest text-orange-600">📣 Sponsored · Paid timeline placement</div><span className="rounded-full bg-orange-100 px-2.5 py-1 text-[9px] font-black text-orange-700">ACTIVE CAMPAIGN</span></div>{ad.image_url && <a href={ad.image_url} target="_blank" rel="noreferrer"><img src={ad.image_url} alt={ad.title} className="mt-4 max-h-80 w-full rounded-2xl object-cover" /></a>}<h3 className="mt-4 text-xl font-black text-slate-900">{ad.title}</h3>{ad.body&&<p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-600">{ad.body}</p>}<div className="mt-4 flex flex-wrap items-center gap-2"><Button variant="orange" onClick={()=>openAd(ad)}>{ad.cta_label || "View opportunity"}</Button><span className="text-[10px] font-bold text-slate-400">Sponsored visibility is shown only after payment is verified.</span></div></div></Card>)}
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <Card className="border-emerald-200 bg-emerald-50/40 p-6">
          <div className="flex items-start gap-4">
            <div className="grid h-14 w-14 shrink-0 place-items-center rounded-2xl bg-emerald-700 text-2xl text-white">🏢</div>
            <div className="min-w-0 flex-1">
              <div className="text-[10px] font-black uppercase tracking-widest text-emerald-700">Saved business profile</div>
              <h2 className="mt-1 text-xl font-black text-slate-900">{currentBusiness?.business_name || currentBusiness?.name || "Your business"}</h2>
              <p className="mt-1 text-xs leading-5 text-slate-500">Your business profile was created during sign-up. You do not need to create another profile inside the portal.</p>
            </div>
          </div>
          {currentBusiness ? <div className="mt-5 grid gap-2 sm:grid-cols-2">
            <div className="rounded-xl bg-white p-3"><div className="text-[9px] font-black uppercase tracking-widest text-slate-400">Location</div><div className="mt-1 text-sm font-black text-slate-800">{currentBusiness.town || "—"}{currentBusiness.province ? ` · ${currentBusiness.province}` : ""}</div></div>
            <div className="rounded-xl bg-white p-3"><div className="text-[9px] font-black uppercase tracking-widest text-slate-400">Email</div><div className="mt-1 truncate text-sm font-black text-slate-800">{currentBusiness.email || authUser?.email || "—"}</div></div>
          </div> : <div className="mt-5 rounded-xl bg-orange-50 p-3 text-xs font-bold text-orange-700">Your saved business profile is still loading. Please refresh if this message remains.</div>}
          <div className="mt-5 flex flex-wrap gap-2">
            <Button variant="green" onClick={() => nav("status")}>🟠 Post Status</Button>
            <Button variant="light" onClick={() => document.getElementById("guardian-advertising")?.scrollIntoView({ behavior: "smooth", block: "start" })}>📣 Sponsored Campaign</Button>
          </div>
        </Card>
  ...
  const renderProfile = () => (
    <div className="space-y-6">
      ...
      <Card className="overflow-hidden">
        <div className="bg-emerald-700 p-6 text-white sm:p-8"><div className="flex flex-wrap items-center gap-5"><button type="button" onClick={()=>nav("timeline")} className="rounded-full focus:outline-none focus:ring-2 focus:ring-orange-300" aria-label="Open my active statuses">{statusAvatar(myStatusName, myStatusPhoto, activeMyStatusCount, "h-20 w-20")}</button><div><div className="text-2xl font-black">{currentPerson?.name || "GUARDIAN WORK member"}</div><div className="text-emerald-100">{account?.role === "business" ? "Business workspace" : currentPerson?.job_title || "Work identity"}</div><div className="mt-2 text-xs text-emerald-100">{[currentPerson?.town,currentPerson?.province].filter(Boolean).join(" - ")}</div></div></div></div>
  ...
  const renderSettings = () => (
    <div className="space-y-6">
      ...
      <div className="text-xs text-slate-400">GUARDIAN WORK V6.9.1 - Real-time Chat + Status + Payment Methods</div>
    </div>
  );
