...
export default function Page() {
  ...
  const recordSecurityEvent = async (eventType: string, metadata: Record<string, any> = {}) => {
    if (!authUser?.id) return;
    const { error } = await supabase.from("guardian_security_events").insert({
      auth_user_id: authUser.id,
      event_type: eventType,
      metadata,
    });
    if (error) console.warn("Security event could not be recorded", error.message);
  };

  const loadSecurityEvents = async () => {
    if (!authUser?.id) return;
    const { data } = await supabase
      .from("guardian_security_events")
  ...
  const exportMyData = async () => {
    if (!authUser?.id) return flash("Please sign in first.");
    setSecurityBusy(true);
    try {
      const [personRes, businessRes, accountRes, workIdRes, proofsRes, applicationsRes, savedRes, preferenceRes] = await Promise.all([
        supabase.from("Job seekers").select("*").eq("auth_user_id", authUser.id).maybeSingle(),
        ...
        currentPerson ? supabase.from("guardian_work_ids").select("*").eq("job_seeker_id", currentPerson.id) : Promise.resolve({ data: [] } as any),
        currentPerson ? supabase.from("guardian_work_proofs").select("*").eq("job_seeker_id", currentPerson.id) : Promise.resolve({ data: [] } as any),
        currentPerson ? supabase.from("applications").select("*").eq("job_seeker_id", currentPerson.id) : Promise.resolve({ data: [] } as any),
        currentPerson ? supabase.from("saved_opportunities").select("*").eq("job_seeker_id", currentPerson.id) : Promise.resolve({ data: [] } as any),
        currentPerson ? supabase.from("notification_preferences").select("*").eq("job_seeker_id", currentPerson.id).maybeSingle() : Promise.resolve({ data: null } as any),
      ]);
      const payload = {
        exported_at: new Date().toISOString(),
        account: accountRes.data || null,
        profile: personRes.data || businessRes.data || null,
        work_identity: workIdRes.data || [],
        proofs: proofsRes.data || [],
        applications: applicationsRes.data || [],
        saved_opportunities: savedRes.data || [],
        notification_preferences: preferenceRes.data || null,
      };
      const blob = new Blob([JSON.stringify(payload, null, 2)], { type: "application/json" });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `guardian-work-data-${new Date().toISOString().slice(0,10)}.json`;
      document.body.appendChild(a);
      a.click();
      a.remove();
      URL.revokeObjectURL(url);
      await recordSecurityEvent("data_exported");
      await loadSecurityEvents();
      flash("Your available GUARDIAN WORK data has been exported.");
    } finally {
  ...
  const requestAccountDeletion = async () => {
    if (!authUser?.id) return flash("Please sign in first.");
    if (!window.confirm("Request account deletion? This starts a deletion review; it does not immediately erase your account.")) return;
    setDeleteRequestBusy(true);
    const { error } = await supabase.from("guardian_account_deletion_requests").insert({
      auth_user_id: authUser.id,
    ...
    await loadSecurityEvents();
    setDeleteRequestBusy(false);
    flash("Deletion request recorded. Your account remains protected while the request is reviewed.");
  };
  ...
  useEffect(() => {
    if (authUser?.id) loadSecurityEvents();
  }, [authUser?.id]);
      ...
      <div className="mx-auto flex min-h-[85vh] max-w-md items-center">
        <Card className="w-full overflow-hidden">
          <div className="bg-emerald-700 p-7 text-white">
            ...
            <div className="mt-5 text-xs font-black uppercase tracking-widest text-orange-300">GUARDIAN WORK V6.6</div>
  ...
  const renderAlerts = () => {
    ...
    return (
      <div className="space-y-6">
        <div>
          <div className="text-xs font-black uppercase tracking-widest text-orange-500">V6.5 - GUARDIAN ALERTS</div>
  ...
  const renderSettings = () => (
    <div className="space-y-6">
      ...
      <Card className="p-6">
        <div className="text-xs font-black uppercase tracking-widest text-emerald-600">Security & data</div>
        <h2 className="mt-1 font-black text-slate-900">You control your GUARDIAN WORK data.</h2>
        <p className="mt-2 text-sm leading-6 text-slate-500">Export the information available to your account, review recent security events, or request account deletion.</p>
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          <Button variant="green" disabled={securityBusy} onClick={exportMyData}>{securityBusy ? "Preparing export..." : "Export my data"}</Button>
          <Button variant="danger" disabled={deleteRequestBusy} onClick={requestAccountDeletion}>{deleteRequestBusy ? "Submitting..." : "Request account deletion"}</Button>
        </div>
        <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-4">
          <div className="font-black text-slate-800">Recent security activity</div>
          <div className="mt-3 space-y-2">
            {securityEvents.map(event => <div key={event.id} className="flex items-center justify-between gap-3 text-xs"><span className="font-bold text-slate-600">{event.event_type.replaceAll("_", " ")}</span><span className="text-slate-400">{formatDate(event.created_at)}</span></div>)}
            {!securityEvents.length && <div className="text-xs text-slate-400">No security events recorded yet.</div>}
          </div>
        </div>
      </Card>
      ...
      <div className="text-xs text-slate-400">GUARDIAN WORK V6.6 - Security + Data - Secure Accounts - Smart Discovery - Saved Work</div>
    </div>
  );
    ...
    <div className={`guardian-work ${theme === "dark" ? "guardian-dark" : ""} min-h-screen bg-slate-50 text-slate-900`}>
      ...
      <footer className="border-t border-slate-200 bg-white">
        <div className="mx-auto flex max-w-7xl flex-col gap-2 px-4 py-8 text-xs text-slate-500 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div>V6.6 - Security + Data · No Dead Buttons</div>
        </div>
      </footer>
