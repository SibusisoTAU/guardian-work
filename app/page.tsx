...
export default function Page() {
      ...
      <div className="mx-auto flex min-h-[85vh] max-w-md items-center">
        <Card className="w-full overflow-hidden">
          <div className="bg-emerald-700 p-7 text-white">
            ...
            <div className="mt-5 text-xs font-black uppercase tracking-widest text-orange-300">GUARDIAN WORK V6.3</div>
          ...
          <div className="p-6">
            ...
            <div className="mt-5 text-center text-xs text-slate-500">V6.3 adds Work ID, proof of ability and stronger discovery.</div>
          </div>
        </Card>
      </div>
    </div>
    ...
    <div className="min-h-screen bg-slate-50 px-4 py-8">
      <div className="mx-auto flex min-h-[85vh] max-w-xl items-center">
        <Card className="w-full p-6 sm:p-8">
          <div className="text-xs font-black uppercase tracking-widest text-orange-500">V6.3 setup</div>
        ...
        <div className="grid gap-8 lg:grid-cols-[1.4fr_.6fr] lg:items-center">
          <div>
            <div className="mb-3 inline-flex rounded-full bg-orange-400 px-3 py-1 text-xs font-black text-slate-950">
              V6.3 - WORK ID + SOCIAL TIMELINE + MATCH ENGINE
            </div>
      ...
      <Card className="overflow-hidden border-emerald-200">
        <div className="bg-gradient-to-r from-emerald-700 to-emerald-600 p-6 text-white">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <div className="text-xs font-black uppercase tracking-widest text-emerald-100">GUARDIAN WORK ID · V6.3</div>
  ...
  const renderMatches = () => (
    <div className="space-y-6">
      <div>
        <div className="text-xs font-black uppercase tracking-widest text-orange-500">V6.3 Match Engine</div>
            ...
            <Fragment key={post.id}>
              <Card className="overflow-hidden">
                <div className="p-5">
                  ...
                  {(comments[post.id] || []).length > 0 && <div className="mt-3 space-y-2 border-t border-slate-100 pt-3">{(comments[post.id] || []).slice(-5).map(c => <div key={c.id} className="rounded-xl bg-slate-50 p-3 text-xs leading-5 text-slate-600"><b className="text-slate-800">{appPerson(c.author_id)?.name || "Member"}</b> · {c.content}</div>)}</div>}
                </div>
              </Card>
              {index % 3 === 2 && targetedAds.length > 0 && <div className="space-y-3">{targetedAds.slice(0, 1).map(ad => <Card key={`${post.id}-${ad.id}`} className="overflow-hidden border-orange-200 bg-orange-50/40"><div className="p-5"><div className="flex items-center justify-between gap-3"><span className="rounded-full bg-orange-500 px-2.5 py-1 text-[10px] font-black uppercase text-white">Sponsored</span><span className="text-[10px] font-bold text-slate-400">Smart discovery</span></div><h3 className="mt-3 font-black text-slate-900">{ad.title}</h3>{ad.body && <p className="mt-1 text-sm leading-6 text-slate-600">{ad.body}</p>}<a href={ad.cta_url} target="_blank" rel="noreferrer" onClick={() => recordAdEvent(ad.id, "click")} className="mt-4 inline-flex rounded-xl bg-orange-500 px-4 py-2.5 text-sm font-black text-white">{ad.cta_label}</a></div></Card>)}</div>}
            </Fragment>
          );
        })}

        {!visiblePosts.length && <Card className="p-10 text-center"><div className="text-4xl">🌱</div><h2 className="mt-3 font-black text-slate-900">Your timeline is ready.</h2><p className="mt-1 text-sm text-slate-500">Be the first to share something useful about your work.</p></Card>}
      </div>
    );
  };
  ...
  const renderAlerts = () => {
    ...
    return (
      <div className="space-y-6">
        <div>
          <div className="text-xs font-black uppercase tracking-widest text-orange-500">V6.3 - GUARDIAN ALERTS</div>
  ...
  const renderSettings = () => (
    <div className="space-y-6">
      ...
      <div className="text-xs text-slate-400">GUARDIAN WORK V6.3 - Match Engine - Secure Accounts - Smart Discovery - Saved Work</div>
    </div>
  );
    ...
    <div className={`guardian-work ${theme === "dark" ? "guardian-dark" : ""} min-h-screen bg-slate-50 text-slate-900`}>
      ...
      <footer className="border-t border-slate-200 bg-white">
        <div className="mx-auto flex max-w-7xl flex-col gap-2 px-4 py-8 text-xs text-slate-500 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div>V6.3 - Work ID, Proof & Work Circles</div>
        </div>
      </footer>
